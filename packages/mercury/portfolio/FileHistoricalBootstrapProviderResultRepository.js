...
export function createHistoricalBootstrapProviderResult({operation,sourceId="DATAFORSEO_GOOGLE_SHOPPING",providerTaskId,paidActionIntentId,checkpointId,atlasProductId,productIndex,sourceRightsProfileDigest,providerStatus="AVAILABLE",operationResult,retrievedAt}={}){
 if(!["PRODUCTS","PRODUCT_INFO","SELLERS"].includes(operation)||![sourceId,providerTaskId,paidActionIntentId,checkpointId,atlasProductId,retrievedAt].every(x=>typeof x==="string"&&x.trim())||!/^[a-f0-9]{64}$/.test(sourceRightsProfileDigest??"")||!Number.isInteger(productIndex)||productIndex<0||!Number.isFinite(Date.parse(retrievedAt))||operationResult==null)throw new Error("BOOTSTRAP_PROVIDER_RESULT_INVALID");
 const identity={operation,sourceId,providerTaskId,paidActionIntentId,sourceRightsProfileDigest},resultDigest=digest(operationResult),canonicalResultId=`mer_providerresult_${digest(identity).slice(0,24)}`;
