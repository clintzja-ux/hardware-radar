...
export function createHistoricalRefreshAuthorizationRequest({refreshPlan,sourceValidation,createdAt=new Date().toISOString(),ttlMinutes=15,spentTodayUsd=0,dailySpendCeilingUsd=DATAFORSEO_DEFAULT_UTC_DAY_SPEND_CEILING_USD}={}){
 validateRefreshPlan(refreshPlan);if(!Number.isFinite(Date.parse(createdAt))||!Number.isInteger(ttlMinutes)||ttlMinutes<=0)throw new TypeError("HISTORICAL_REFRESH_AUTHORIZATION_TIME_INVALID");if(!Number.isFinite(spentTodayUsd)||spentTodayUsd<0)throw new Error("HISTORICAL_REFRESH_CURRENT_DAY_SPEND_INVALID");if(!Number.isFinite(dailySpendCeilingUsd)||dailySpendCeilingUsd<=0||Math.round((spentTodayUsd+.001+Number.EPSILON)*1e9)/1e9>dailySpendCeilingUsd)throw new Error("HISTORICAL_REFRESH_DAILY_BUDGET_EXCEEDED");
