...
import { ATLAS_PRE_EXPANSION_PRODUCT_IDS } from "../../atlas/AtlasPreExpansionLifecycleReview.js";

let cases=0;const asOf="2026-09-03T14:00:00.000Z",readJson=async resource=>JSON.parse(await readFile(resource,"utf8")),products=new ProductRepository({readJson}),canonicalProducts=await products.getAll(),preparedProducts=structuredClone(canonicalProducts);for(const product of preparedProducts.filter(item=>item.identity.createdBy==="system:atlas-ram-expansion-002"||ATLAS_PRE_EXPANSION_PRODUCT_IDS.includes(item.identity.atlasProductId))){product.governance.lifecycleStatus="DRAFT";product.governance.publicationStatus="PENDING";product.governance.humanReviewRequired=true;}const preparedAtlas={products:{getAll:async()=>structuredClone(preparedProducts)}};
...
const root=await mkdtemp(join(tmpdir(),"hr-portfolio-production-"));try{
 ...
 assert.equal(first.portfolio.costEnvelope.currentUtcDaySpendUsd,.002);assert.equal(first.portfolio.costEnvelope.remainingUtcDayCapacityUsd,.023);cases++;
 ...
 const currentProducts=structuredClone(canonicalProducts);
 const driftService=new ProductionAcquisitionPortfolioPrepareService({atlas:{products:{getAll:async()=>structuredClone(currentProducts)}},rightsRegistry:defaultSourceRightsRegistry,providerIdentityResolver:identity,executionRepository:ledger,portfolioRepository:repository});
 const driftValidation=await driftService.validate(stored);assert.equal(driftValidation.valid,true);assert.equal(driftValidation.atlasDriftSincePrepare,true);assert.equal(driftValidation.portfolio.counts.eligible,11);assert.equal(driftValidation.portfolio.taskEnvelope.maximumProgramTaskCount,first.portfolio.taskEnvelope.maximumProgramTaskCount);assert.equal(driftValidation.portfolio.costEnvelope.maximumProgramSpendUsd,first.portfolio.costEnvelope.maximumProgramSpendUsd);cases++;
 const future=await driftService.domain.prepare({asOf:"2026-09-04T14:00:00.000Z",currentUtcDaySpendUsd:0});assert.equal(future.counts.eligible,103);assert.equal(future.portfolioCycleId===stored.portfolioCycleId,false);cases++;
 const changed=structuredClone(first.portfolio);changed.actualSpendUsd=1;await assert.rejects(()=>repository.persist(changed),/IMMUTABLE_CONFLICT/);await assert.rejects(()=>service.validate(changed),/IMMUTABLE_BINDING_INVALID/);cases++;
 const tampered=structuredClone(stored);tampered.eligibleProducts.push(structuredClone(tampered.eligibleProducts[0]));await assert.rejects(()=>service.validate(tampered),/IMMUTABLE_BINDING_INVALID/);cases++;
