...
const root=await mkdtemp(join(tmpdir(),"hr-portfolio-production-"));try{
 ...
 assert.equal(first.portfolio.counts.canonicalProducts,26);assert.equal(first.portfolio.counts.eligible,11);assert.equal(first.portfolio.counts.excluded,15);cases++;
 ...
 assert.equal((await new ProductRepository({readJson}).getAll()).length,26);cases++;
