...
const root=await mkdtemp(join(tmpdir(),"hr-portfolio-production-"));try{
 ...
 const future=await driftService.domain.prepare({asOf:"2026-09-04T14:00:00.000Z",currentUtcDaySpendUsd:0});assert.equal(future.counts.eligible,88);assert.equal(future.portfolioCycleId===stored.portfolioCycleId,false);cases++;
