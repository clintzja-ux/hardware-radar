...
assert.equal(portfolio.costEnvelope.utcDaySpendCeilingUsd, .05); cases++;
assert.equal(portfolio.costEnvelope.maxTasksPerUtcDay, 50); cases++;
...
assert(projectAcquisitionPortfolio({ portfolio, currentUtcDaySpendUsd: .05 }).blockers.includes("UTC_DAY_SPEND_CEILING_EXHAUSTED")); cases++;
