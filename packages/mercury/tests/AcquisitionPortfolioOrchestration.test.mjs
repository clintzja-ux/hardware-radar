...
assert.equal(portfolio.costEnvelope.utcDaySpendCeilingUsd, .075); cases++;
assert.equal(portfolio.costEnvelope.maxTasksPerUtcDay, 75); cases++;
assert.equal(portfolio.costEnvelope.minimumUtcDayCapacityEnvelopes, 1); cases++;
...
assert(projectAcquisitionPortfolio({ portfolio, currentUtcDaySpendUsd: .075 }).blockers.includes("UTC_DAY_SPEND_CEILING_EXHAUSTED")); cases++;
