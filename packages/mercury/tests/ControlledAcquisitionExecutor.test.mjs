...
const makePlan=(id="a", costs=[.001,.001],spentTodayUsd=0)=>createAcquisitionPlan({policy,plannedAt:"2026-08-17T06:00:00.000Z",spentTodayUsd,candidates:costs.map((cost,i)=>({candidateId:`${id}-${i}`,priority:"HIGH",estimatedCostUsd:cost,execution:{kind:"FAKE_PAID_POST",key:`${id}-${i}`}}))});
let calls=0;
...
const costlyPlan=makePlan("costly",[.001,.001,.001],.002); let costlyCalls=0;
...
let failureCalls=0; const failurePlan=makePlan("failure",[.001,.001],.0055);
