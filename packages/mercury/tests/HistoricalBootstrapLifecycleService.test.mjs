...
const service=new HistoricalBootstrapLifecycleService({artifactRepository:{getById:async()=>null},checkpointRepository:repository,continuationRepository:{record:async value=>(savedAuthorization=value),getById:async()=>savedAuthorization},prepareOwner:{prepare:async()=>({reference:{}})},handoff:{execute:async input=>(calls++,{status:"PAID_TASK_CREATED",...input})},resultBridge:{retrieve:async()=>({status:"PENDING"})},resultDispatcher:{dispatch:async()=>({})},rightsRegistry:{get:()=>({acquisition:{api:"ALLOWED"}})},spendResolver:async()=>0,now:()=>"2026-09-11T12:01:00Z"});
