...
const mercury=new Mercury(); const observation=makeAmazonApiObservation("mer_obs_000000700","2026-08-10T15:00:00Z");
...
const result=evaluateLiveMarketEligibility(observation,{product,retailer,freshness,confidence,evaluatedAt:asOf});
assert.equal(result.eligible,true); assert.deepEqual(result.reasons,[]);
console.log("Live market eligibility tests passed.");
