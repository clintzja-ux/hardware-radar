...
try{
  ...
  const sellers=["Microless.com","AAAwave","Grooves-Inc.com","directmacro.com","ServerOrbit.com","MemoryC.com","Hard Disk Direct","NextWarehouse.com","Micro Center","CompSource"];
  const cohort=sellers.map((name,index)=>{const value=structuredClone(retainedAmbiguous);value.evidenceId=`dfev_crucial_${String(index).padStart(2,"0")}`;value.idempotencyKey=`crucial-${index}`;value.candidate.marketEvidence.seller={...value.candidate.marketEvidence.seller,name,domain:`seller-${index}.example`,url:`https://seller-${index}.example/item`};value.candidate.marketEvidence.provenance.rawPayloadReference=`dataforseo:sellers:${sellersTaskId}:item:${index}`;value.candidate.marketEvidence.pricing.shippingPrice=[0,2].includes(index)?10+index:null;value.candidate.marketEvidence.offer.availability=index>=8?"out_of_stock":"in_stock";return value;});
  const retentionAudit={schemaVersion:"1.0",operation:"SELLERS_RESULT_DF003_RETENTION",sellersTaskId,productInfoTaskId,integrations:cohort.map(record=>({evidenceId:record.evidenceId})),governance:governed,atlasProductId:proposal.atlasProductId};
  const cohortSnapshot=structuredClone(cohort);const composition=composeInitialAcquisitionPromotionAssessment({records:cohort,requestedAtlasProductId:proposal.atlasProductId,retentionAudit,sellersProposalEnvelope:{schemaVersion:"1.0",proposal},atlasProduct});assert.equal(composition.mode,"GOVERNED_INITIAL_ACQUISITION_BINDING");assert.equal(composition.records.length,10);assert.equal(composition.initialAcquisitionIdentityProjections.length,10);assert.ok(composition.initialAcquisitionIdentityProjections.every(value=>value.atlasProductId===proposal.atlasProductId&&value.df003Outcome==="CONFIRMED"));
  for(const [index,record] of composition.records.entries()){const assessment=assessDataForSeoEvidencePromotion({records:[record],initialAcquisitionIdentityProjections:[composition.initialAcquisitionIdentityProjections[index]]});assert.equal(assessment.productIdentity,"VERIFIED");assert.equal(assessment.merchantIdentity,"DISCOVERED");assert.equal(assessment.state,"REVIEW_REQUIRED");assert.equal(assessment.historicalEligible,false);assert.equal(record.candidate.marketEvidence.offer.condition,null);assert.equal(record.candidate.marketEvidence.pricing.shippingPrice,[0,2].includes(index)?10+index:null);assert.equal(record.candidate.marketEvidence.offer.availability,index>=8?"out_of_stock":"in_stock");}
  assert.deepEqual(cohort,cohortSnapshot);
  const legacy=composeInitialAcquisitionPromotionAssessment({records:[stored],requestedAtlasProductId:proposal.atlasProductId});assert.equal(legacy.mode,"GENERIC_RESOLVER");assert.equal(legacy.records.length,1);assert.equal(legacy.initialAcquisitionIdentityProjections.length,0);
  assert.throws(()=>composeInitialAcquisitionPromotionAssessment({records:cohort,requestedAtlasProductId:proposal.atlasProductId,retentionAudit:{...retentionAudit,integrations:[...retentionAudit.integrations,{evidenceId:cohort[0].evidenceId}]},sellersProposalEnvelope:{proposal},atlasProduct}),/BINDING_CONFLICT/);
  assert.throws(()=>composeInitialAcquisitionPromotionAssessment({records:cohort,requestedAtlasProductId:proposal.atlasProductId,retentionAudit:{...retentionAudit,integrations:retentionAudit.integrations.slice(1)},sellersProposalEnvelope:{proposal},atlasProduct}),/EVIDENCE_SET_CONFLICT/);
  assert.throws(()=>composeInitialAcquisitionPromotionAssessment({records:cohort,requestedAtlasProductId:proposal.atlasProductId,retentionAudit,sellersProposalEnvelope:{proposal:{...proposal,validationDigest:"invalid"}},atlasProduct}),/PROPOSAL_BINDING/);
  const drift=structuredClone(cohort);drift[0].candidate.marketEvidence.productEvidence.productId="drift";assert.throws(()=>composeInitialAcquisitionPromotionAssessment({records:drift,requestedAtlasProductId:proposal.atlasProductId,retentionAudit,sellersProposalEnvelope:{proposal},atlasProduct}),/PROVIDER_IDENTITY_CONFLICT/);
  assert.throws(()=>composeInitialAcquisitionPromotionAssessment({records:cohort,requestedAtlasProductId:proposal.atlasProductId,retentionAudit,sellersProposalEnvelope:{proposal},atlasProduct:{...atlasProduct,governance:{...atlasProduct.governance,lifecycleStatus:"DRAFT"}}}),/ACTIVE_READY/);
  const conflict=structuredClone(cohort);conflict[0].candidate.marketEvidence.offer.details="Crucial 32GB DDR4 1x32GB";assert.throws(()=>composeInitialAcquisitionPromotionAssessment({records:conflict,requestedAtlasProductId:proposal.atlasProductId,retentionAudit,sellersProposalEnvelope:{proposal},atlasProduct}),/SELLER_CONTRADICTION/);
  const evidenceState=join(root,"cohort-evidence.json"),decisionState=join(root,"cohort-decisions.json"),retentionState=join(root,"cohort-retention.json"),proposalState=join(root,"cohort-proposal.json");await writeFile(evidenceState,JSON.stringify({version:"1.0",records:Object.fromEntries(cohort.map(record=>[record.evidenceId,record])),idempotency:{}}),"utf8");await writeFile(decisionState,JSON.stringify({version:"1.0",sequence:0,decisions:{},bySubject:{},idempotency:{},remediationSequence:0,remediations:{},remediationByDecision:{},remediationIdempotency:{}}),"utf8");await writeFile(retentionState,JSON.stringify(retentionAudit),"utf8");await writeFile(proposalState,JSON.stringify({schemaVersion:"1.0",proposal}),"utf8");
  const output=execFileSync(process.execPath,["scripts/mercury-evidence-promotion-assess.mjs",`--state=${evidenceState}`,`--decision-state=${decisionState}`,`--atlas-product=${proposal.atlasProductId}`,`--retention-state=${retentionState}`,`--sellers-proposal=${proposalState}`],{cwd:process.cwd(),encoding:"utf8",stdio:["ignore","pipe","pipe"]});assert.match(output,/Evidence count:\s+10/);assert.match(output,/Product identity:\s+VERIFIED/);assert.match(output,/Merchant identity:\s+DISCOVERED/);assert.match(output,/Composition mode:\s+GOVERNED_INITIAL_ACQUISITION_BINDING/);assert.match(output,/"identitySource":"GOVERNED_INITIAL_ACQUISITION_BINDING"/);assert.match(output,/"merchantReason":"ATLAS_RETAILER_NOT_FOUND"/);assert.match(output,/Paid task created:\s+NO/);assert.match(output,/Actual spend:\s+\$0\.000/);
}finally{await rm(root,{recursive:true,force:true});}
console.log("B-009A/B-010A governed initial-acquisition promotion composition tests passed (60 cases).");
