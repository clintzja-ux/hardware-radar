...
import {
  ...
  createBoundedSpendProgressionResolver, createProductionAmazonProductsDiscoverySourceOwner, createProductionGoogleProductsDiscoverySourceOwner,
...
try{
  ...
  const google=createProductionGoogleProductsDiscoverySourceOwner({atlas,boundedRepository,childArtifactRepository,taskLedger,executionRepository,consumptionRepository,resultRepository,rightsRegistry:defaultSourceRightsRegistry,boundedSpendProgressionResolver,acquisitionService:googleProvider,stateRoot:root,now:()=>at});
  ...
  const amazon=createProductionAmazonProductsDiscoverySourceOwner({atlas,destinationRepository:{getAll:async()=>[{destinationId:"amazon-destination",atlasProductId:amazonProduct.identity.atlasProductId,retailerId:"RETAILER-0001",marketplace:"amazon.com",retailerListingId:"B0CQQVNCB6",status:"ACTIVE"}]},historicalRepository:{getAll:async()=>[]},boundedRepository,artifactRepository:amazonArtifactRepository,actionRepository:amazonActionRepository,taskLedger,executionRepository,consumptionRepository,resultRepository,rightsRegistry:defaultSourceRightsRegistry,spendResolver,boundedSpendProgressionResolver,acquisitionService:amazonProvider,stateRoot:root,now:()=>at});
