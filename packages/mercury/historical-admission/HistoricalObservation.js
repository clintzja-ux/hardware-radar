...
export function validateHistoricalObservation(record){
  ...
  if(acquisitionType==="MANUAL_PUBLISHER_OBSERVATION"){for(const key of ["preparationId","preparationDigest","sourceId","sourceRightsProfileDigest","originalPreparationRightsDigest","admissionRightsProfileDigest","originalRightsLineageState","destinationId","destinationBindingDigest","evidenceReference"])if(!nonBlank(acquisition?.[key]))errors.push(`Historical manual observation ${key} is required.`);if(acquisition.providerTaskId!=null||acquisition.productsTaskId!=null||acquisition.productInfoTaskId!=null||acquisition.sellersTaskId!=null)errors.push("Historical manual observation must not invent provider tasks.");}
  if(acquisitionType==="INITIAL_ACQUISITION"){for(const key of ["productsTaskId","productInfoTaskId","sellersTaskId"])if(!nonBlank(acquisition?.[key]))errors.push(`Historical ${key} is required.`);}else if(acquisitionType==="AMAZON_INITIAL_ACQUISITION"){for(const key of ["productsTaskId","sellersTaskId","governedAsin","immutableSellersResultId","immutableSellersResultDigest","sourceRightsProfileDigest"])if(!nonBlank(acquisition?.[key]))errors.push(`Historical Amazon ${key} is required.`);}else if(acquisitionType==="HISTORICAL_REFRESH"){for(const key of ["sellersTaskId","refreshPlanId","authorizationId","identityReuseAssessmentId","sourceEvidenceId"])if(!nonBlank(acquisition?.[key]))errors.push(`Historical refresh ${key} is required.`);if(acquisition.productsTaskId!=null||acquisition.productInfoTaskId!=null)errors.push("Historical refresh must not invent PRODUCTS or PRODUCT_INFO tasks.");if(!acquisition.providerIdentity||!["productId","dataDocId","gid"].some(key=>nonBlank(acquisition.providerIdentity[key])))errors.push("Historical refresh provider identity is required.");}else if(acquisitionType==="REUSABLE_IDENTITY_REPEAT"){for(const key of ["atlasProductId","sourceId","operation","reusableIdentityRecordId","reusableIdentityDigest","sourceRightsProfileDigest","preparedObservationId","repeatAuthorizationId","paidActionIntentId","acquisitionCycleId","observationCycle","providerTaskId","canonicalResultId","canonicalResultDigest","lineageBindingDigest"])if(!nonBlank(acquisition?.[key]))errors.push(`Historical repeat ${key} is required.`);if(acquisition.productsTaskId!=null||acquisition.productInfoTaskId!=null)errors.push("Historical repeat must not invent PRODUCTS or PRODUCT_INFO tasks.");}else if(acquisitionType!=="MANUAL_PUBLISHER_OBSERVATION")errors.push("Historical acquisition type is invalid.");
