...
export function validateGovernedInitialAcquisitionIdentityProjection(projection){
  ...
  for(const field of ["rawPayloadReference","atlasProductId","sourceProductsTaskId","productInfoTaskId","sellersTaskId","upstreamValidationDigest","retentionGovernanceDigest"])if(typeof projection?.[field]!=="string"||!projection[field])errors.push(`INITIAL_ACQUISITION_${field.toUpperCase()}_INVALID`);
...
export function createGovernedInitialAcquisitionIdentityProjection({governance,sellersProposal,atlasProduct,sellerItem=null,record=null,evidenceId=record?.evidenceId??null,rawPayloadReference=record?.candidate?.marketEvidence?.provenance?.rawPayloadReference??null}={}){
  ...
  if(sellersProposal?.atlasProductId!==lineage.atlasProductId||sellersProposal.sourceProductsTaskId!==lineage.sourceProductsTaskId||sellersProposal.sourceProductInfoTaskId!==lineage.productInfoTaskId||sellersProposal.sourceProductInfoExecutionRunId!==lineage.sourceProductInfoExecutionRunId||sellersProposal.validationDigest!==hash(validation))throw new Error("INITIAL_ACQUISITION_PROPOSAL_BINDING_INVALID");
  ...
  if(!Array.isArray(comparisons)||!comparisons.some(entry=>entry.authorizationToProductInfo==="MATCH")||comparisons.some(entry=>[entry.authorizationToProductInfo,entry.authorizationToSellers,entry.productInfoToSellers].includes("CONTRADICTION")))throw new Error("INITIAL_ACQUISITION_PROVIDER_IDENTITY_INVALID");
  ...
  const base={schemaVersion:"1.0",projectionType:"GOVERNED_INITIAL_ACQUISITION_BINDING",status:"APPLICABLE",evidenceId:evidenceId??null,rawPayloadReference,atlasProductId:lineage.atlasProductId,sourceProductsTaskId:lineage.sourceProductsTaskId,productInfoTaskId:lineage.productInfoTaskId,sellersTaskId:lineage.sellersTaskId,providerIdentity:structuredClone(expected),upstreamValidationDigest:sellersProposal.validationDigest,retentionGovernanceDigest:hash(governance),sellerAssessment:{status:sellerAssessment.status,contradictions:structuredClone(sellerAssessment.contradictions),evidence:structuredClone(sellerAssessment.evidence)},df003Outcome:"CONFIRMED",effectiveProductState:"VERIFIED"};
