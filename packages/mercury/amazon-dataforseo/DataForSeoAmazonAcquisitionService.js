...
async function persist(ledger,request,task,lineage){const {paidActionIntentId=null,checkpointId=null,productIndex=null,sourceRightsProfileDigest=null,acquisitionCycleId=null,preparedObservationId=null,reusableIdentityDigest=null,repeatAuthorizationId=null,parentRunAuthorizationId=null}=lineage;const requestKey=key({sourceId:request.sourceId,operation:request.operation,requestDigest:request.requestDigest,paidActionIntentId});ledger.requireNew(requestKey);return ledger.record(requestKey,{kind:request.operation,taskId:task.id,costUsd:Number(task.cost??0),createdStatus:task.status_code,sourceId:request.sourceId,atlasProductId:request.internalLineage.atlasProductId,asin:request.providerPayload.asin??null,requestDigest:request.requestDigest,checkpointId,productIndex,sourceRightsProfileDigest,...(paidActionIntentId?{paidActionIntentId}:{}),...(acquisitionCycleId?{acquisitionCycleId}:{}),...(preparedObservationId?{preparedObservationId}:{}),...(reusableIdentityDigest?{reusableIdentityDigest}:{}),...(repeatAuthorizationId?{repeatAuthorizationId}:{}),...(parentRunAuthorizationId?{parentRunAuthorizationId}:{})});}
