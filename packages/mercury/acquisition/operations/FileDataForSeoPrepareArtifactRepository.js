...
export function createDataForSeoPrepareArtifact({operation,source="DATAFORSEO_GOOGLE_SHOPPING",atlasProductId,productIndex=null,paidActionIntentId=null,bootstrapCheckpointId=null,bootstrapArtifactId=null,sourceRightsProfileDigest=null,originatingCheckpointSequence=null,prepareArtifact,authorizationRequest,preparedAt}={}){
 ...
 const bootstrap=paidActionIntentId!=null;if(bootstrap&&(!/^mer_histbootintent_[a-f0-9]{24}$/.test(paidActionIntentId)||!bootstrapCheckpointId||!bootstrapArtifactId||!/^[a-f0-9]{64}$/.test(sourceRightsProfileDigest??"")||!Number.isInteger(originatingCheckpointSequence)||originatingCheckpointSequence<1||!Number.isInteger(productIndex)||productIndex<0))throw new Error("PREPARE_ARTIFACT_BOOTSTRAP_BINDING_INVALID");
 const logical={operation,source,atlasProductId,productIndex,paidActionIntentId,bootstrapCheckpointId,bootstrapArtifactId,sourceRightsProfileDigest,originatingCheckpointSequence},material={...logical,prepareArtifact,authorizationRequest,preparedAt},bindingDigest=digest(material),prepareArtifactId=`mer_prepare_${digest(logical).slice(0,24)}`;
