...
export class AcquisitionPortfolioPrepareService {
  ...
  async prepare({ asOf, locationName = "United States", languageName = "English", currentUtcDaySpendUsd = 0 } = {}) {
    if (!iso(asOf) || !nonBlank(locationName) || !nonBlank(languageName)) throw new TypeError("ACQUISITION_PORTFOLIO_PREPARE_INPUT_INVALID");
    validateMoney(currentUtcDaySpendUsd, "currentUtcDaySpendUsd");
    ...
    for (const product of products) {
      ...
      const state = resolved?.status ?? "ABSENT";
      if (!['REUSABLE','NOT_REUSABLE','REVIEW_REQUIRED','ABSENT'].includes(state) || (state === 'REUSABLE' && !validProviderIdentity(resolved))) throw new Error(`ACQUISITION_PORTFOLIO_PROVIDER_IDENTITY_INVALID:${material.atlasProductId}`);
      const reusable = state === "REUSABLE", reviewRequired = state === "REVIEW_REQUIRED", stagePath = reusable ? ["SELLERS"] : reviewRequired ? [] : ["PRODUCTS", "PRODUCT_INFO", "SELLERS"];
      eligibleProducts.push({ atlasProductId: material.atlasProductId, manufacturerPartNumber: material.manufacturerPartNumber, atlasProductDigest: digest(material), query: material.manufacturerPartNumber, providerIdentityState: state, providerIdentity: reusable ? { productId: resolved.productId ?? null, dataDocId: resolved.dataDocId ?? null, gid: resolved.gid ?? null, bindingDigest: resolved.bindingDigest } : null, initialState: reusable ? PORTFOLIO_PRODUCT_STATES.READY_FOR_SELLERS : reviewRequired ? PORTFOLIO_PRODUCT_STATES.PRODUCTS_REVIEW_REQUIRED : PORTFOLIO_PRODUCT_STATES.READY_FOR_PRODUCTS, plannedStagePath: stagePath, maximumRemainingPaidTasks: stagePath.length, maximumRemainingSpendUsd: money(stagePath.length * this.taskCostUsd) });
    }
