...
export function createSellersEnrichmentProposal({productInfoTaskId,productInfoResult,productInfoAuthorization,createdAt=new Date().toISOString(),estimatedCostUsd=.001}={}){
  ...
  const item=productInfoResult?.result?.[0]?.items?.[0];
  if(!item) throw new Error('SELLERS_REQUIRES_PRODUCT_INFO_EVIDENCE');
  ...
  const stable=(expected.dataDocId&&actual.dataDocId===expected.dataDocId)||(expected.productId&&actual.productId===expected.productId)||(expected.gid&&actual.gid===expected.gid);
  if(!stable) throw new Error('SELLERS_PRODUCT_INFO_IDENTITY_MISMATCH');
