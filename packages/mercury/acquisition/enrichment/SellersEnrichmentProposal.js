...
export function createDirectProductsSellersProposal({atlasProduct,productsReview,sourceRightsDigest,provider='DATAFORSEO',source='DATAFORSEO_GOOGLE_SHOPPING',locationName='United States',languageName='English',createdAt=new Date().toISOString(),estimatedCostUsd=.001}={}){
  ...
  const candidate=resolution.recommendedCandidate,anchor=routing.providerAnchor??candidate?.item,providerIdentity={productId:anchor?.productId??null,dataDocId:anchor?.dataDocId??null,gid:anchor?.gid??null};
  ...
  return freeze({schemaVersion:'1.2',identityLineageType:'DIRECT_PRODUCTS_STRONG_IDENTITY',proposalId:`sellerenrich_${materialDigest.slice(0,24)}`,status:'PENDING_OPERATOR_REVIEW',createdAt,operation:'SELLERS',atlasProductId:atlasProduct.identity.atlasProductId,canonicalMpn:atlasProduct.identity.manufacturerPartNumber,sourceProductsTaskId:productsReview.providerTaskId,sourceProductsReviewId:productsReview.reviewId,sourceProductsReviewMaterialDigest:productsReview.materialDigest,sourceProductInfoTaskId:null,sourceProductInfoAuthorizationId:null,sourceProductInfoProposalId:null,providerIdentity,providerGrouping:routing.providerIdentity?.groupingKey??null,evidence:{title:candidate?.item?.title??null,specifications:[],sellerCount:null},governanceBinding:{sourceRightsDigest,provider,source,locationName,languageName},validation,validationDigest,materialDigest,estimatedCostUsd,maxPaidTasks:1,automaticPaidRetries:0,authorizationCreated:false});
}
