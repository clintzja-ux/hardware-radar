...
export class ProductInfoResultRetrievalService{
  ...
  async retrieve({taskId,productInfoAuthorization,productInfoProposal,taskLedger,executionRuns,authorizationConsumptions,atlasProduct,brandAliases=[]}={}){
    ...
    const materialDigest=digest(material),record=freeze({schemaVersion:"1.0",resultId:`mer_productinforesult_${materialDigest.slice(0,24)}`,...material,retrievedAt:derivedAt,materialDigest,status:"PRODUCT_INFO_VALIDATED",paidTaskCreated:false,actualSpendUsd:0,downstreamAuthority:{sellersExecution:false,retention:false,historical:false,canonical:false,review:false,publication:false,currentPrice:false,cheapest:false,pick:false}});
    if(prior&&prior.materialDigest!==materialDigest)throw new Error("PRODUCT_INFO_RESULT_REPLAY_CONFLICT");
    const persisted=await this.resultRepository.record(record);return freeze({status:persisted.status==="DUPLICATE"?"DUPLICATE":"RESULT_RECEIVED",result:persisted.result,sellersReadiness:"READY_FOR_SELLERS",sellersProposal:derived,lineage,paidTaskCreated:false,actualSpendUsd:0});
  }
}
