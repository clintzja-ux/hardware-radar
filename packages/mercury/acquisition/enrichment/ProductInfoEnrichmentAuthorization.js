...
export function createProductInfoEnrichmentAuthorizationRequest({proposal,createdAt=new Date().toISOString(),ttlMinutes=15,spentTodayUsd=0,dailySpendCeilingUsd=DATAFORSEO_DEFAULT_UTC_DAY_SPEND_CEILING_USD}={}){
  ...
  if(!Number.isFinite(dailySpendCeilingUsd)||dailySpendCeilingUsd<=0||Math.round((spentTodayUsd+.001+Number.EPSILON)*1e9)/1e9>dailySpendCeilingUsd) throw new Error('PRODUCT_INFO_DAILY_BUDGET_EXCEEDED');
  ...
  if(proposal.paidActionIntentId) binding.paidActionIntentId=proposal.paidActionIntentId;
  ...
  if(proposal.bootstrapBinding) binding.bootstrapBinding=proposal.bootstrapBinding;
  const proposalDigest=hash(binding);
  const planId=`enrichplan_${hash([proposalDigest,createdAt]).slice(0,24)}`;
  const execution={kind:'PRODUCT_INFO',...providerIdentity,locationName:proposal.governanceBinding?.providerRequest?.locationName??'United States',languageName:proposal.governanceBinding?.providerRequest?.languageName??'English',...(proposal.paidActionIntentId?{paidActionIntentId:proposal.paidActionIntentId}:{})};
  const plan=freeze({schemaVersion:'1.0',planId,plannedAt:createdAt,policy:{schemaVersion:'1.0',enabled:true,maxPaidTasksPerRun:1,maxSpendPerRunUsd:0.001,maxSpendPerDayUsd:dailySpendCeilingUsd,automaticPaidRetries:0},spentTodayUsd,approvedTaskCount:1,estimatedApprovedSpendUsd:0.001,decisions:[{candidateId:`enrichment:${proposal.atlasProductId}:${proposal.proposalId}`,priority:'NORMAL',estimatedCostUsd:0.001,decision:'APPROVED',reason:null,rationale:'Operator-reviewed E2C product identity recommendation.',execution}]});
  const requestId=`enrichauth_${hash([planId,proposalDigest]).slice(0,24)}`;
  const request={schemaVersion:'1.0',requestId,mode:'LIVE',authorizationType:'PRODUCT_INFO_ENRICHMENT',planId,proposalId:proposal.proposalId,proposalDigest,sourceTaskId:proposal.sourceTaskId,atlasProductId:proposal.atlasProductId,providerIdentity,...(proposal.paidActionIntentId?{paidActionIntentId:proposal.paidActionIntentId}:{}),createdAt,expiresAt:new Date(Date.parse(createdAt)+ttlMinutes*60000).toISOString(),maxSpendUsd:0.001,maxPaidTasks:1,automaticPaidRetries:0,status:'PENDING_OPERATOR_APPROVAL',plan};if(proposal.providerSelectionLineage)request.providerSelectionLineage=proposal.providerSelectionLineage;if(proposal.governanceBinding)request.governanceBinding=proposal.governanceBinding;if(proposal.bootstrapBinding)request.bootstrapBinding=proposal.bootstrapBinding;return freeze(request);
}
...
export function assertProductInfoAuthorizationBinding({request,proposal}={}){
  ...
  const providerIdentity={productId:proposal?.providerIdentity?.productId??null,dataDocId:proposal?.providerIdentity?.dataDocId??null,gid:proposal?.providerIdentity?.gid??null};
  const binding={proposalId:proposal?.proposalId,sourceTaskId:proposal?.sourceTaskId,atlasProductId:proposal?.atlasProductId,operation:proposal?.operation,providerIdentity};if(proposal?.paidActionIntentId)binding.paidActionIntentId=proposal.paidActionIntentId;if(proposal?.providerSelectionLineage)binding.providerSelectionLineage=proposal.providerSelectionLineage;if(proposal?.governanceBinding)binding.governanceBinding=proposal.governanceBinding;if(proposal?.bootstrapBinding)binding.bootstrapBinding=proposal.bootstrapBinding;const digest=hash(binding);
  ...
  if(JSON.stringify(request.bootstrapBinding??null)!==JSON.stringify(proposal?.bootstrapBinding??null)) throw new Error('PRODUCT_INFO_BOOTSTRAP_BINDING_MISMATCH');
