...
function binding(p){
 const value=p?.identityLineageType!=='DIRECT_PRODUCTS_STRONG_IDENTITY'?{proposalId:p?.proposalId,atlasProductId:p?.atlasProductId,sourceProductsTaskId:p?.sourceProductsTaskId,sourceProductInfoTaskId:p?.sourceProductInfoTaskId,sourceProductInfoAuthorizationId:p?.sourceProductInfoAuthorizationId,operation:p?.operation,providerIdentity:identity(p)}:{proposalId:p?.proposalId,identityLineageType:p.identityLineageType,atlasProductId:p?.atlasProductId,sourceProductsTaskId:p?.sourceProductsTaskId,sourceProductsReviewId:p?.sourceProductsReviewId??null,sourceProductsReviewMaterialDigest:p?.sourceProductsReviewMaterialDigest??null,sourceProductInfoTaskId:null,sourceProductInfoAuthorizationId:null,validationDigest:p?.validationDigest??null,operation:p?.operation,providerIdentity:identity(p)};if(p?.paidActionIntentId)value.paidActionIntentId=p.paidActionIntentId;if(p?.bootstrapBinding)value.bootstrapBinding=p.bootstrapBinding;return value;
}
export function createSellersEnrichmentAuthorizationRequest({proposal,createdAt=new Date().toISOString(),ttlMinutes=15,spentTodayUsd=0,dailySpendCeilingUsd=DATAFORSEO_DEFAULT_UTC_DAY_SPEND_CEILING_USD}={}){
 ...
 if(!Number.isFinite(dailySpendCeilingUsd)||dailySpendCeilingUsd<=0||Math.round((spentTodayUsd+.001+Number.EPSILON)*1e9)/1e9>dailySpendCeilingUsd) throw new Error('SELLERS_DAILY_BUDGET_EXCEEDED');
 ...
 const execution={kind:'SELLERS',...providerIdentity,locationName:proposal.governanceBinding?.locationName??'United States',languageName:proposal.governanceBinding?.languageName??'English',...(proposal.paidActionIntentId?{paidActionIntentId:proposal.paidActionIntentId}:{})};
 const plan=freeze({schemaVersion:'1.0',planId,plannedAt:createdAt,policy:{schemaVersion:'1.0',enabled:true,maxPaidTasksPerRun:1,maxSpendPerRunUsd:.001,maxSpendPerDayUsd:dailySpendCeilingUsd,automaticPaidRetries:0},spentTodayUsd,approvedTaskCount:1,estimatedApprovedSpendUsd:.001,decisions:[{candidateId:`sellers:${proposal.atlasProductId}:${proposal.proposalId}`,priority:'NORMAL',estimatedCostUsd:.001,decision:'APPROVED',reason:null,rationale:direct?'Certified direct PRODUCTS strong identity.':'Operator-reviewed governed Product Info identity.',execution}]});
 const requestId=`sellerauth_${hash([planId,proposalDigest]).slice(0,24)}`;
 const common={schemaVersion:direct?'1.1':'1.0',requestId,mode:'LIVE',authorizationType:'SELLERS_ENRICHMENT',planId,proposalId:proposal.proposalId,proposalDigest,atlasProductId:proposal.atlasProductId,sourceProductsTaskId:proposal.sourceProductsTaskId,sourceProductInfoTaskId:proposal.sourceProductInfoTaskId??null,sourceProductInfoAuthorizationId:proposal.sourceProductInfoAuthorizationId??null,providerIdentity,...(proposal.paidActionIntentId?{paidActionIntentId:proposal.paidActionIntentId}:{}),createdAt,expiresAt:new Date(Date.parse(createdAt)+ttlMinutes*60000).toISOString(),maxSpendUsd:.001,maxPaidTasks:1,automaticPaidRetries:0,status:'PENDING_OPERATOR_APPROVAL',plan};
 if(proposal.bootstrapBinding)common.bootstrapBinding=proposal.bootstrapBinding;return freeze(direct?{...common,identityLineageType:proposal.identityLineageType,sourceProductsReviewId:proposal.sourceProductsReviewId,sourceProductsReviewMaterialDigest:proposal.sourceProductsReviewMaterialDigest}:common);
}
export function assertSellersAuthorizationBinding({request,proposal}={}){
 ...
 if(JSON.stringify(request.bootstrapBinding??null)!==JSON.stringify(proposal?.bootstrapBinding??null))throw new Error('SELLERS_BOOTSTRAP_BINDING_MISMATCH');
