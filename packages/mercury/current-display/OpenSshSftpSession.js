...
import { classifyOpenSshFailure, redactRakutenSftpError } from "./RakutenSftpConfig.js";
...
export class OpenSshSftpSession {
    constructor({config,knownHostsPath,askpassPath,trustOnFirstUse=false,spawnImpl=spawn,sftpExecutable=process.platform==="win32"?path.join(process.env.SystemRoot??"C:\\Windows","System32","OpenSSH","sftp.exe"):"sftp",sshExecutable=process.platform==="win32"?path.join(process.env.SystemRoot??"C:\\Windows","System32","OpenSSH","ssh.exe"):"ssh"}={}){this.config=config;this.knownHostsPath=path.resolve(knownHostsPath);this.askpassPath=path.resolve(askpassPath);this.trustOnFirstUse=trustOnFirstUse;this.spawnImpl=spawnImpl;this.sftpExecutable=sftpExecutable;this.sshExecutable=sshExecutable;this.process=null;this.buffer="";this.stderr="";this.waiters=[];this.secrets=[config?.username,config?.password];}
    async connect(){
        ...
        await this.requireFile(this.askpassPath,"SFTP_SESSION_START_FAILED");
        if(process.platform==="win32"&&!/\.(?:cmd|bat|exe)$/i.test(this.askpassPath))throw new Error("SFTP_SESSION_START_FAILED");
        await mkdir(path.dirname(this.knownHostsPath),{recursive:true});
        const args=["-P",String(this.config.port),"-o",`UserKnownHostsFile=${this.knownHostsPath}`,"-o","StrictHostKeyChecking=accept-new","-o","BatchMode=no","-o","PreferredAuthentications=password,keyboard-interactive","-o","PubkeyAuthentication=no","-o","NumberOfPasswordPrompts=1",`${this.config.username}@${this.config.host}`];
        ...
        const append=chunk=>{this.buffer+=chunk.toString("utf8");this.flush();}; this.process.stdout.on("data",append);this.process.stderr.on("data",chunk=>{this.stderr+=chunk.toString("utf8");});
        this.process.on("error",error=>this.rejectAll(this.failure(error)));
        this.process.on("exit",code=>{if(code!==0)this.rejectAll(this.failure(this.stderr));});
    ...
    failure(value){const code=classifyOpenSshFailure(value);return Object.assign(new Error(code),{code});}
    ...
    waitForPrompt(code){return new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(this.failure(this.stderr)||Object.assign(new Error(code),{code})),30000);this.waiters.push({resolve:value=>{clearTimeout(timer);resolve(value);},reject:error=>{clearTimeout(timer);reject(error);}});this.flush();});}
