...
import {BoundedExistingTaskProgressionService} from "../bounded/BoundedExistingTaskProgression.js";
...
export class RepeatObservationBoundedDomainAdapter{
  ...
  async advanceMember({member,authorization,createChildBinding,checkpoint}){
    ...
    const processed=await this.repeatService.processRetain({preparedObservationId:current.domainMemberId,canonicalResultId:retrieved.canonicalResultId}),evidenceIds=processed.evidenceIds??[],history=[],skippedEvidence=[];
    for(const evidenceId of evidenceIds)try{const assessed=await this.repeatService.assessAdmitFact({evidenceId,action:"ASSESS",admittedBy:null});if(assessed.historicalEligible!==true){skippedEvidence.push({evidenceId,reason:"HISTORICAL_ADMISSION_NOT_ELIGIBLE"});continue;}const admitted=await this.repeatService.assessAdmitFact({evidenceId,action:"ADMIT",admittedBy:`machine:${authorization.authorizationId}`});if(admitted.observationId)history.push(admitted.observationId);}catch(error){const message=String(error?.message??error);if(message.startsWith("HISTORICAL_ADMISSION_NOT_ELIGIBLE:")){skippedEvidence.push({evidenceId,reason:message});continue;}throw error;}
    if(!history.length&&skippedEvidence.length)throw new Error(skippedEvidence[0].reason);
    return{...current,state:BOUNDED_MEMBER_STATES.COMPLETED,evidenceIds,historicalObservationIds:history,skippedEvidence,duplicates:processed.duplicates??0};
  }
...
export class BoundedRepeatObservationRunService{
  constructor({repeatService,repository,spendResolver=async()=>0,now=()=>new Date().toISOString(),dailySpendCeilingUsd=DATAFORSEO_DEFAULT_UTC_DAY_SPEND_CEILING_USD}={}){if(!repeatService||!repository)throw new Error("REPEAT_RUN_DEPENDENCIES_REQUIRED");this.repository=repository;this.adapter=new RepeatObservationBoundedDomainAdapter({repeatService});this.coordinator=new NeutralBoundedPaidActionCoordinator({repository:new RepeatBoundedPersistenceAdapter(repository),domainAdapter:this.adapter,spendResolver,now,dailySpendCeilingUsd,policyVersion:BOUNDED_REPEAT_RUN_POLICY_VERSION,idPrefixes:{plan:"mer_repeatrunplan",authorization:"mer_repeatrunauth",run:"mer_repeatrun"}});this.pendingProgression=new BoundedExistingTaskProgressionService({coordinator:this.coordinator,now});}
  async prepareRun(input={}){exact(input,["observationCycle","cohort","selectionProvenance"],"REPEAT_RUN_PREPARE_INPUT_INVALID");const record=await this.coordinator.prepare({cycle:input.observationCycle,cohort:input.cohort,selectionProvenance:input.selectionProvenance??null});return result(record,planToLegacy);}
  ...
  async progressExistingRun(input={}){exact(input,["runId","maxAttempts","intervalMs"],"REPEAT_RUN_PROGRESS_INPUT_INVALID");return this.pendingProgression.progress(input);}
