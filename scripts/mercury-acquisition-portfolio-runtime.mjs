...
import { defaultSourceRightsRegistry, FileAcquisitionExecutionLedgerRepository, FileDataForSeoMarketEvidenceRepository, FileHistoricalObservationRepository, FileAcquisitionPortfolioRepository, GovernedProviderIdentityResolver, ProductionAcquisitionPortfolioPrepareService } from "../packages/mercury/index.js";
...
export function createProductionPortfolioRuntime(args,{readJson=async resource=>JSON.parse(await readFile(resource instanceof URL?fileURLToPath(resource):resource,"utf8"))}={}){
  ...
  return {service,portfolioRepository,productRepository,evidenceRepository,historicalRepository,executionRepository,providerIdentityResolver};
}
