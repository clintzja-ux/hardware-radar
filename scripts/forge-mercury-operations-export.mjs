...
const cohortOperationsProvider=new ProductionForgeCohortOperationsProvider({boundedDatabasePath:location("--bounded-state",path.join(mercury,"products-identity-discovery","identity-discovery.sqlite")),repeatDatabasePath:location("--repeat-state",path.join(mercury,"repeat-observations.sqlite"))});
...
const operationalContextProvider={load:async({evidenceRecords})=>{const [plan,authorization,execution,consumption,result]=await Promise.all([readOptional(paths.plan,null),readOptional(paths.authorization,null),readOptional(paths.execution,{schemaVersion:"1.0",runs:[]}),readOptional(paths.consumption,{schemaVersion:"1.0",consumed:[]}),readOptional(paths.result,null)]),governed=await resolveGovernedHistoricalRefreshContext({refreshResult:result,refreshPlan:plan,evidenceRepository,evidenceRecords});return {refreshPlans:plan?[plan]:[],authorizations:authorization?[authorization]:[],executionRuns:execution.runs??[],consumptionState:consumption,refreshResults:governed.refreshResult?[governed.refreshResult]:[],identityReuseAssessments:governed.identityReuseAssessments};}};
const exported=await new CertifiedMercuryOperationsExporter({productRepository,retailerRepository,evidenceRepository,historicalRepository,identityDecisionRepository,canonicalObservationRepository,reviewRepository,publicationRepository,cadencePolicyRepository,operationalContextProvider,cohortOperationsProvider}).export({asOf,outputPath:paths.output});
const cohorts=exported.projection.cohortOperations?.cohorts??[],memberCount=cohorts.reduce((total,cohort)=>total+cohort.members.length,0);
console.log("CERTIFIED MERCURY OPERATIONS EXPORT");console.log("");console.log("Artifact:               ",exported.artifactPath);console.log("Schema:                 ",exported.projection.schemaVersion);console.log("As of:                  ",exported.projection.asOf);console.log("Cohorts:                ",cohorts.length);console.log("Cohort members:         ",memberCount);console.log("Atlas products:         ",exported.projection.products.length);console.log("Historical semantics:  ",exported.projection.semantics.historicalValueSemantics);console.log("Source state modified:  NO");console.log("Network operation:      NONE");console.log("Paid task created:      NO");console.log("Actual spend:           $0.000");
