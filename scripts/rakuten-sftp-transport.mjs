...
let result;try{result=operation==="inspect"?await transport.inspect():await transport.downloadAndValidate({signal:controller.signal,onProgress:reportProgress});}catch(error){console.error("RAKUTEN PRODUCT CATALOG SFTP TRANSPORT");console.error("Operation:                   ",error.code??"SFTP_OPERATION_FAILED");printTransfer(error.transfer);printDirectories(error.directoryListings);printDiscovery(error.discovery);printAccounting(error.connectionAccounting);console.error("Actual spend:                $0.000");process.exitCode=1;throw error;}finally{process.removeListener("SIGINT",cancel);process.removeListener("SIGTERM",cancel);}
...
if(result.status==="DOWNLOADED_AND_VALIDATED"){
 printTransfer(result.transfer);console.log("Header timestamp:           ",result.headerTimestamp);console.log("Product rows:               ",result.productRows);console.log("Trailer rows:               ",result.trailerRows);console.log("Modification I/U/D:         ",`${result.modifications.I}/${result.modifications.U}/${result.modifications.D}`);console.log("Field counts:               ",result.fieldCounts.join(", "));console.log("Gzip/parser integrity:      PASS");
}
