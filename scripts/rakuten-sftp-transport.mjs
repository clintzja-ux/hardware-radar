...
let result;try{result=operation==="inspect"?await transport.inspect():await transport.downloadAndValidate();}catch(error){console.error("RAKUTEN PRODUCT CATALOG SFTP TRANSPORT");console.error("Operation:                   ",error.code??"SFTP_OPERATION_FAILED");printDirectories(error.directoryListings);printDiscovery(error.discovery);printAccounting(error.connectionAccounting);console.error("Actual spend:                $0.000");process.exitCode=1;throw error;}
