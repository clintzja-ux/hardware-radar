...
assert.equal(new Set(catalog.products.map((item) => item.atlasProductId)).size, 22);
...
assert.equal(filter({ query: "6000z30" }).length, 1);
...
assert.equal(filter(browserModule.EMPTY_RAM_CATALOG_FILTERS).length, 22);
...
console.log("GROWTH-002 static Atlas RAM catalog contract passed (22 products). ");
