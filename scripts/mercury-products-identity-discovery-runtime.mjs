...
export async function runIdentityDiscoveryCommand(action,values,{runtime,readJson=async file=>JSON.parse(await readFile(file,"utf8"))}={}){const args=parseIdentityDiscoveryArgs(action,values),service=(runtime??createProductionProductsIdentityDiscoveryService()).service;let result;
  ...
  else result=await service.resume({runId:args.get("--run-id"),resumedBy:args.get("--resumed-by"),confirmation:args.get("--confirm")});
  return result;
}
