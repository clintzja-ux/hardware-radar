...
import { selectorProposalCohort, validateScaleRampSelectorProposal } from "../packages/mercury/scale-ramp/ScaleRampSelectorProposal.js";

const specifications=Object.freeze({
  prepare:{allowed:["--cohort-file","--selector-proposal-file","--discovery-cycle"],required:["--discovery-cycle"]},
...
export async function runIdentityDiscoveryCommand(action,values,{runtime,readJson=async file=>JSON.parse(await readFile(file,"utf8"))}={}){const args=parseIdentityDiscoveryArgs(action,values),owner=runtime??createProductionProductsIdentityDiscoveryService(),service=owner.service;let result;
  if(action==="prepare"){const cohortFile=args.get("--cohort-file"),proposalFile=args.get("--selector-proposal-file");if(Boolean(cohortFile)===Boolean(proposalFile))throw new Error("IDENTITY_DISCOVERY_PREPARE_SOURCE_REQUIRED");if(proposalFile){const resolved=path.resolve(proposalFile),selectionProvenance=validateScaleRampSelectorProposal(await readJson(resolved),{reference:proposalFile});result=await service.prepare({cycle:args.get("--discovery-cycle"),cohort:selectorProposalCohort(selectionProvenance),selectionProvenance});}else{const cohort=await readJson(path.resolve(cohortFile));if(!Array.isArray(cohort)||cohort.some(row=>!row||Object.keys(row).some(key=>!["atlasProductId","sourceId"].includes(key))||typeof row.atlasProductId!=="string"||typeof row.sourceId!=="string"))throw new Error("IDENTITY_DISCOVERY_COHORT_INVALID");result=await service.prepare({cycle:args.get("--discovery-cycle"),cohort});}}
