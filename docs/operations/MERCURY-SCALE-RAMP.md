...
Current classification: `100_PRODUCT_MEMBERSHIP_AND_READINESS_READY_FOR_CHECKPOINT`.
...
The eight-member proposal covers all five manufacturers, DDR4/DDR5, DIMM/SO-DIMM, 8/16/32 GB, one-/two-module configurations, and eight primary strata. This is descriptive evidence, not a manual-selection criterion. At the September 17 review timestamp, durable daily spend was zero; eight tasks at `$0.0015` project a `$0.0120` maximum under the `$0.0500` ceiling. The artifact grants no PREPARE or paid authority. After checkpointing, the next separate increment is fresh revalidation, PREPARE, INSPECT, and STOP before authorization.

The exact proposal passed fresh selector replay and member revalidation and was prepared through the existing Products identity-discovery owner using [100-product-first-amazon-products-canary-cohort.json](../../config/mercury/scale-ramp/100-product-first-amazon-products-canary-cohort.json). Plan `mer_iddiscplan_420db0ac62da881c02c8c61d` (`420db0ac62da881c02c8c61dcfa73927d1a03f6d0b72c27f9b90d2c195281ef5`) is requested 8 / READY 8 / blocked 0. Fresh authorization-time checks preserved the selector lineage, exact membership, rights, destinations, zero spend, and the 8-task / `$0.0120` envelope. Single-use parent authorization `mer_iddiscauth_51e1d2b264818db31b3fefe5` (`51e1d2b264818db31b3fefe5349453e71a2b931de13fc3b886c04f6bef1085ff`) was created at `2026-09-17T00:41:53.504Z` and expires at `2026-09-17T01:01:52.8535330+00:00`. Final state is `AUTHORIZED / NOT_STARTED`, with zero child authorities, tasks, provider calls, or spend for this plan. The next separate action is checkpointing; START requires explicit operator approval and must not be inferred from this authorization increment.
...
Maximum derived demand is up to 65 Amazon Products tasks (`$0.0975`), 22 Amazon Sellers tasks (`$0.0330`), and two Google Sellers tasks (`$0.0020`), separately governed. The theoretical `$0.1325` total needs at least three `$0.0500` UTC-day windows and is not a proposed authorization. September 16 durable spend is `$0.0080`. No canonical rule selects a paid canary from the 65 eligible discovery members, so exact canary membership remains an operator decision after matrix review; no PREPARE may run yet.
