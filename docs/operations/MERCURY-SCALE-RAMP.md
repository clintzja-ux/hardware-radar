...
The reserved final six remain exactly the committed deterministic partition. Current reassessment at `2026-09-15T22:51:43.893Z` found all 6 `READY_FOR_DISCOVERY` with unchanged Amazon rights and active reviewed destinations. Cohort `config/mercury/scale-ramp/25-product-slice-2-amazon-identity.json` binds plan `mer_iddiscplan_e0ec35a7e9f99bd15f824e60`: six tasks / `$0.0090`, `$0.0235` current-day spend, and `$0.0325` projected final spend under the `$0.0500` ceiling. Single-use authorization `mer_iddiscauth_7521266859ea7c8df1ee71da` is `AUTHORIZED` until `2026-09-15T23:12:04.9798952Z`; the run remains `NOT_STARTED`. Authorization performed no provider operation or paid work. START requires separate operator review and explicit authorization. The separate longitudinal control should wait until `AFTER_25_IDENTITY_STAGE` so it does not compete for the final breadth slice's budget.
