...
Fresh canonical readiness is 41 reusable Amazon identities, 57 identity-absent products, and two H052-governed products. Twelve identity-absent operations are prior-paid terminal/non-repurchase cases and 12 lack a reviewed Amazon destination. The repeat owner resolves 41 Amazon and two Google reusable source/product pairs. A materially larger repeat/Sellers cohort would add the missing evidence for task/retrieval volume, seller and shipping normalization, evidence/history amplification, duplicate-free historical admission, persistence growth, P1-A behavior, Forge comprehension, and exception workload. However, repeat scale selection is not yet Hardware Radar-owned: `mercury:repeat-run:prepare` accepts `--cohort-file`, while the certified selector/proposal and direct proposal binding are Products-identity-discovery-specific. The next zero-provider P1 must add deterministic repeat candidate selection and immutable review-proposal provenance over product + source + operation + observation cycle, reusing current owners and granting no PREPARE, authorization, or spend authority. Proposal-to-PREPARE binding remains the following boundary unless the smallest compatible implementation can safely certify both without changing ownership.
...
The next engineering increment is the narrow zero-provider direct-binding P1. Existing identity-discovery PREPARE should accept the validated immutable review proposal and additively bind stage ID/digest, selector policy/version, candidate-pool digest, selected-member digest, proposal digest/reference, source/operation, and exact membership into the existing plan. Current Atlas identity, readiness, rights, destinations, and conflicts must still be revalidated. No new repository, selector, paid authorization automation, scheduler, or Forge redesign belongs in that closure.
