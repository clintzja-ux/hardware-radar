...
## Post-25 longitudinal canary projection

For repeat cohorts, Forge lifecycle counts reflect original canonical member state: `COMPLETED` is completed, `EXCEPTION` is a terminal exception, and only genuinely nonterminal members are pending. Governed immutable-result reprocessing is displayed separately and never rewrites the original run. For run `mer_repeatrun_e3d2417b9e15058c2b44b649`, the operator confirmed the supported HTTP-served view reports requested 6 / completed 3 / pending 0 / exceptions 3, preserves the original exceptions, separately shows effective Amazon reprocessing effects, reports `Systemic: NO`, and grants no downstream authority. D2 is `IMPLEMENTED / FIXTURE-CERTIFIED / OPERATOR-VISIBILITY-CONFIRMED`.
...
Current classification: `100_PRODUCT_STAGE_COMPLETE / LONGITUDINAL_GATE_CLOSED / NO_OPEN_STAGE_P0_OR_P1 / NO_DOWNSTREAM_AUTHORITY`.

The formal whole-stage assessment closes the fixed 100-product RAM scale stage. The cohort remains 100 unique `ACTIVE + READY` Atlas products with digest `470dfadbdb8670de3566826d33eb74cb9d97a1ca6e14bf8b3fac71101ecaaf21`; real Products and repeat operations, bounded money controls, immutable results, 204 retained evidence records, 188 historical observations, additive recovery, supervised P1-A progression, Forge schema 1.1 visibility, and fail-closed publication separation satisfy the canonical stage requirements. There is no separate promotion repository or artifact: the reviewed canonical current/stage record is the promotion boundary. Completion does not authorize acquisition, publication, Current Price, Cheapest, Picks, merge, or deployment.

Retained work is classified rather than erased. Provider timing, persistence growth, exception demand, and operating cadence remain `MEASURE`. Prepared/authorized Forge visibility, richer exception/review queues, reduced command/ID ceremony, background scheduling, durable cross-run metrics, and evidence-triggered storage migration remain `DEFERRED_BEFORE_1000`. Bounded Beacon/Gateway production connection, governed live Amazon publication, explicit curated Rakuten/Newegg publication, public measurement, RAM Market Overview, continued market-memory accumulation, and the next Atlas scale stage are `POST_100_PRODUCT_WORK`.

### Repeat scale selection and review proposal

`IC-MERCURY-REPEAT-SCALE-SELECTION-001` is `PRODUCTION-OPERATIONALLY-PROVEN`. Policy `MERCURY_REPEAT_SCALE_SELECTION / 1.0` uses the shared scale-ramp canonical digest primitive with repeat-specific candidate semantics: exact Atlas product, source, Sellers operation, explicit canonical observation cycle, source-owned identity/rights/destination bindings, and stable Atlas stratification attributes. Exact same-cycle duplicates fail closed; historical participation in a different cycle is not a permanent exclusion. Source readiness and current-cycle conflicts remain with existing repeat owners.

`IC-MERCURY-REPEAT-PROPOSAL-PREPARE-001` is also `PRODUCTION-OPERATIONALLY-PROVEN`. `mercury:repeat-run:prepare -- --selector-proposal-file=<review-proposal>` validates the proposal, derives its exact cycle and selected product/source/operation tuples, revalidates current repeat readiness, and writes a provenance-bound zero-authority plan. Do not also supply `--cohort-file`; an optional duplicate `--observation-cycle` must match exactly. Legacy cohort input remains available for legitimate non-selector workflows.
...
The real zero-authority experiment gate is represented by [100-stage-larger-longitudinal-repeat-policy.json](../../config/mercury/scale-ramp/100-stage-larger-longitudinal-repeat-policy.json), [100-stage-larger-longitudinal-amazon-review.json](../../config/mercury/scale-ramp/100-stage-larger-longitudinal-amazon-review.json), and [100-stage-larger-longitudinal-google-control-review.json](../../config/mercury/scale-ramp/100-stage-larger-longitudinal-google-control-review.json). Fresh owners produced 41 Amazon and two Google candidates at cycle `2026-09-18T00:00:00.000Z`. The Amazon proposal selects 24; the Google proposal selects both controls. Both remain immutable `REVIEW_ONLY / authority NONE` proposals. Production Amazon direct PREPARE created plan `mer_repeatrunplan_a5195b970ff9a01e05bee625`, binding digest `a5195b970ff9a01e05bee625efdbf4ae7b4662e3d30b9ba42f00847a1ac095c5`, and cohort digest `a29e4700f5b33f8b2d900eb642d2cf0348da908f53f9f6e28d016c856eee8616`. Production Google direct PREPARE created independent plan `mer_repeatrunplan_77a5d4c60a49f86718d495da`, binding digest `77a5d4c60a49f86718d495dae0f874641f4cba0e3aa9e881b1024cfcf46436ed`, and cohort digest `d6094caa6daef96c744ff99cee00a85dd88d547eb578c35c613f6cfc8c8ea8af`. Canonical INSPECT proves exact proposal-to-plan membership equivalence for both: Amazon requested 24 / READY 24 / blocked 0 at `$0.0360`; Google requested 2 / READY 2 / blocked 0 at `$0.0020`; both have zero retries, `NOT_AUTHORIZED`, and `NOT_STARTED`. The independently governed aggregate envelope is 26 tasks / `$0.0380`; it creates no mixed-source execution authority.
...
The operator subsequently approved and executed both independent START decisions before their shared `2026-09-18T02:00:00.000Z` expiry, then separately authorized one certified bounded P1-A progression over only the 26 existing tasks. Amazon run `mer_repeatrun_743625479d2d502f313633e6` remains `COMPLETED_WITH_EXCEPTIONS` at 14 completed / zero pending / ten member-local item-price-invalid exceptions; Google run `mer_repeatrun_3aa73e62f28e32955cc9734b` remains `COMPLETED` at two completed / zero pending / zero exceptions. D1 production-scale observation found 24 valid structured shipping `current` rows and 84 null-shipping rows with no malformed shape, currency conflict, or money-invalid recurrence. Combined task/execution counts remain 121/124 and spend remains `$0.0380 / $0.0750`; P1-A created zero paid tasks, retries, replacements, or spend. Per-evidence isolation and governed recovery preserve these original outcomes while producing exact effective accounting: 118 Amazon evidence = 108 history + ten null-price retained-only rows, with zero duplicates. Total evidence/history are 204/188. Historical admission, cross-cycle semantics, exception isolation, recovery governance, and operational independence are sufficient for the fixed 100-product stage. Forge remains truthful for original state and grants no downstream authority; detailed recovery presentation remains deferred. The longitudinal gate is closed, and the formal stage-completion assessment is now recorded above.
...
The Amazon and Google direct PREPARE → INSPECT checkpoints are complete. Their shared governance path is production-operationally proven across both source-owned repeat implementations; Google retained its own identity, rights, no-destination, operation, and `$0.0010` task-ceiling semantics. The canonical UTC-day ceiling is now `$0.0750`, raised from `$0.0500` for proportional larger-stage testing capacity. Amazon `$0.0015`, Google `$0.0010`, and zero retries remain unchanged. Authorization, START, provider work, H052, reprocessing, and downstream authority all remain prohibited.

### Deterministic paid-canary selector

Policy `MERCURY_SCALE_RAMP_CANARY_SELECTION / 1.0` is fixture-certified. For one already-eligible source/operation pool, it normalizes an allowlisted Atlas candidate shape; creates primary strata from manufacturer, memory generation, and form factor; orders strata and members through canonical SHA-256 material bound to policy, stage, source, and operation; and selects by deterministic round-robin. Capacity and module count bind within-stratum order without creating sparse primary strata. Requested size is explicit and independent of budget.

The selector grants no authority. Selection is followed by operator review, then separately authorized PREPARE with current readiness/rights/destination validation, then separately authorized START with current spend validation. Any change to strata, hashing, ordering, tie-breaking, traversal, or normalization requires a new policy version. Candidate membership, readiness, or requested-size changes remain V1 inputs and visibly change digests.

Current read-only composition confirms 65 Amazon Products candidates, stage digest `470dfadbdb8670de3566826d33eb74cb9d97a1ca6e14bf8b3fac71101ecaaf21`, and candidate digest `ed24651aa03dc0d71bb2cd692d8fb25218d08a15240e7d216998655b1ade56cb`. Stage policy [100-product-first-amazon-products-canary-policy.json](../../config/mercury/scale-ramp/100-product-first-amazon-products-canary-policy.json) governs requested size eight for the first 100-product Amazon Products canary only. It is not a global default. The exact deterministic zero-authority result is [100-product-first-amazon-products-canary-review.json](../../config/mercury/scale-ramp/100-product-first-amazon-products-canary-review.json), selected digest `0a17e9ba2e71b1adf8f7f29a4896effe01ac991756dc338c5a2ba86bd8534eb1`.

The eight-member proposal covers all five manufacturers, DDR4/DDR5, DIMM/SO-DIMM, 8/16/32 GB, one-/two-module configurations, and eight primary strata. This is descriptive evidence, not a manual-selection criterion. At the September 17 review timestamp, durable daily spend was zero; eight tasks at `$0.0015` projected a `$0.0120` maximum under the `$0.0500` ceiling. The artifact itself granted no PREPARE or paid authority; those later boundaries were separately reviewed and authorized.

The exact proposal passed fresh selector replay and member revalidation and was prepared through the existing Products identity-discovery owner using [100-product-first-amazon-products-canary-cohort.json](../../config/mercury/scale-ramp/100-product-first-amazon-products-canary-cohort.json). Plan `mer_iddiscplan_420db0ac62da881c02c8c61d` (`420db0ac62da881c02c8c61dcfa73927d1a03f6d0b72c27f9b90d2c195281ef5`) remained requested 8 / READY 8 / blocked 0. Single-use parent authorization `mer_iddiscauth_51e1d2b264818db31b3fefe5` was consumed once by run `mer_iddiscrun_3fdb7e107d83f1725c85acd0`. START created exactly eight unique Amazon Products tasks and spent `$0.0120`. One bounded P1-A invocation performed two retrieval passes over the existing tasks and added no paid work. The run reached `COMPLETED` with five `STRONG_UNIQUE_ASIN` and three safely unresolved `MULTIPLE_COMPATIBLE_ASINS` outcomes; there was no review-eligible or systemic failure. Task completion ranged from 27.126 to 91.424 seconds, median 89.520 seconds. Forge reports requested 8 / completed 8 / pending 0 / blocked 0 / exceptions 3, where those exceptions are safely unresolved rather than pending or systemic. Evidence/history and every downstream authority remained unchanged. The next action is a zero-provider result and stage-gate assessment; no larger paid slice is authorized.

The second 100-stage slice exercised the direct proposal path end to end. Plan `mer_iddiscplan_1286590438ea302f73c02e4c`, authorization `mer_iddiscauth_2e066ba46ff88da0102f5ce1`, and run `mer_iddiscrun_7888ea699df2cbbd162b687c` bind the exact deterministic 24-member proposal with no first-slice overlap. One START created 24 unique Amazon Products tasks, spent `$0.0360`, and consumed the authorization once. The first P1-A invocation used three attempts at a 45-second interval over those tasks only; it created no new task or spend and correctly stopped as `PENDING_BOUND_REACHED` with 22 completed and two pending. A later separately authorized zero-paid continuation made one retrieval pass over only those two existing tasks. Both results became available and the run reached `COMPLETED`; no authority, task, retry, replacement, or spend was added. Final outcomes are 14 strong unique, seven multiple-compatible, one variant conflict, one insufficient-evidence H052-reviewable outcome, and one bundle outcome. Forge matches canonical Mercury at requested 24 / completed 24 / pending 0 / exceptions 10, systemic `NO`, and downstream authority `NONE`. Operators must not use manual RESUME, retry, replace, perform H052, or select a third slice under this authority; the next gate is the zero-provider second-slice results / remaining-stage assessment.

The completed remaining-stage assessment finds no material operational reason for a third Products breadth slice before shifting risk dimensions. Across the fixed 100-product stage, the two Products slices exercised 32 operations with 19 strong unique, ten multiple-compatible, one variant conflict, one insufficient-evidence/H052-reviewable result, one bundle, and zero provider/systemic failures, retries, replacements, or unauthorized spend. All five manufacturers, DDR4/DDR5, DIMM/SO-DIMM, 8/16/32/64 GB, and one-/two-module shapes were exercised. The 33 destination-ready/unpaid Products candidates are predominantly further examples of already-proven strata; additional acquisition remains legitimate routine catalog enrichment, not required scale certification.

Fresh canonical readiness is 41 reusable Amazon identities, 57 identity-absent products, and two H052-governed products. Twelve identity-absent operations are prior-paid terminal/non-repurchase cases and 12 lack a reviewed Amazon destination. The repeat owner resolves 41 Amazon and two Google reusable source/product pairs. A materially larger repeat/Sellers cohort would add the missing evidence for task/retrieval volume, seller and shipping normalization, evidence/history amplification, duplicate-free historical admission, persistence growth, P1-A behavior, Forge comprehension, and exception workload. Deterministic repeat selection and direct proposal-to-PREPARE binding are now certified; routine pair copying into a second cohort file is no longer required. The next zero-provider gate owns the real longitudinal experiment policy, cycle, requested size, and whether the two Google pairs form a separate control proposal. It may create review-only proposal(s) but must stop before production PREPARE.

The zero-provider results assessment retains `PASS_WITH_EXPECTED_DOMAIN_EXCEPTIONS`. The prior 19-attempt breadth stage remains distinct from this eight-member canary; their cumulative 27-attempt operating evidence is 19 strong unique, six multiple-compatible, one variant conflict, one insufficient-evidence, and zero provider/systemic failures or retries. It does not establish a population rate. Current Amazon readiness across the fixed 100-product stage is 27 reusable, 60 destination-ready discovery candidates, 12 destination-blocked identity-absent products, and one H052-governed product. Another tiny canary is not justified: the next informative test is materially larger bounded Amazon Products progression after direct selector-proposal → PREPARE lineage is certified.

Proposed next-slice policy, authority `NONE`: continue `DATAFORSEO_AMAZON / AMAZON_PRODUCTS`; request 24 from current eligible candidates through `MERCURY_SCALE_RAMP_CANARY_SELECTION / 1.0`; exclude prior paid slices through canonical current readiness plus reconstructable stage/slice history; cap at 24 tasks / `$0.0360`, zero automatic retries, and one clean `$0.0500` UTC-day envelope; isolate member-local ambiguity while stopping on authority, lineage, spend, rights, persistence, immutable-result, provider-shape, or downstream-leakage integrity failures. Size-16 simulation covers 26.7% of the current pool, 13 strata, and `$0.0240`; size 24 covers 40%, 13 strata, and `$0.0360`; size 32 covers 53.3%, 13 strata, and `$0.0480`. Size 24 provides a threefold scale step without placing the envelope within `$0.0020` of the daily ceiling. No simulation output is a persisted proposal and no members are selected.

The direct-binding P1 is closed without a new repository, selector, paid-authorization mechanism, scheduler, or Forge redesign. The next zero-provider increment may determine the real larger-repeat policy, explicit cycle, requested size, and separate Amazon-primary/Google-control proposal strategy, create immutable review-only proposal(s), and stop before PREPARE.

That P1 is `IMPLEMENTED / FIXTURE-CERTIFIED / PRODUCTION-SHAPED-CERTIFIED` under [IC-MERCURY-SCALE-RAMP-PROPOSAL-PREPARE-001](../implementation%20Contracts/IC-MERCURY-SCALE-RAMP-PROPOSAL-PREPARE-001%20-%20Selector%20Proposal%20to%20PREPARE%20Direct%20Binding.md). The direct CLI consumes the immutable review proposal, derives exact members/source/operation, revalidates current owners, and stores complete provenance inside the existing plan binding. INSPECT exposes that lineage. Legacy plans are not rewritten. The production proof and current gate are recorded below.

The zero-authority second-slice selector result is [100-product-second-amazon-products-slice-review.json](../../config/mercury/scale-ramp/100-product-second-amazon-products-slice-review.json). Fresh readiness produced 60 destination-ready discovery candidates, but three were already-paid unresolved first-slice operations. Applying reconstructable first-slice history as the required non-repurchase exclusion left 57 candidates (`214b1f7bc2baf7839e6d6b0011d6872ece3c6510f3c76c37619cfae8641a81b3`). Selector V1 chose the exact 24 in the artifact (`19814edfccd81e2f8990473c515faa032b8553f5ee4f10befd82e9d531eebe0b`) with zero prior-slice overlap. Validated proposal identity is `mer_scaleproposal_d0bb1c3878f5673e3ead18c0` / `d0bb1c3878f5673e3ead18c0849d19db11fd01d8b5459e6f96eb5a7835c3e878`.

Fresh production PREPARE at cycle `2026-09-17T15:39:53.1264369Z` used that proposal directly and created plan `mer_iddiscplan_1286590438ea302f73c02e4c` (`1286590438ea302f73c02e4cc94d6a9d8910516c6be055133f2908a1c3799772`), cohort digest `85a36ad669d5683e963fca5f7ea27cb53ab862d80e87ef3f51d94fdf190cc5d2`. INSPECT proves complete selector-to-plan provenance and exact membership equivalence: requested 24 / READY 24 / blocked 0, maximum 24 tasks / `$0.0360`, zero retries. After separate operator approval, authorization-time revalidation passed for all exact members, rights, active reviewed Amazon destinations, prior-slice exclusion, conflicts, and the complete daily envelope. Parent authorization `mer_iddiscauth_2e066ba46ff88da0102f5ce1` (`2e066ba46ff88da0102f5ce102bd93072d9f3d1021aa29033fa91068a4c7ab26`) binds this plan and member-set digest, was created `2026-09-17T15:53:08.021Z`, expires `2026-09-17T16:13:07.4277313Z`, permits at most 24 tasks / `$0.0360`, has zero retries, and is single-use for START. Run remains `NOT_STARTED`; no child authority, paid task, provider call, new spend, or downstream state exists. Checkpoint the authorization and stop. START requires separate explicit approval and fresh revalidation. Forge does not currently project prepared/authorized-not-started identity-discovery plans; that supervised-operability enhancement remains non-blocking P2/measurement work.

The stable review-only membership is [100-product-candidate-cohort.json](../../config/mercury/scale-ramp/100-product-candidate-cohort.json): the first 100 validation-clean `ACTIVE + READY` RAM records in canonical Atlas manifest order. Readiness is derived, not embedded in that artifact. At the checkpoint as-of time, Amazon reports 22 reusable, 77 identity-absent, and one H052-review product; 65 of the 77 also satisfy the reviewed-Amazon-destination prerequisite and 12 are destination-blocked before task creation. The canonical repeat resolver owns 22 Amazon and two Google repeat-ready pairs. Google discovery reports 97 discovery-ready, one `NO_USABLE_IDENTITY` review state, and the known two-control discovery-projection limitation, but doctrine forbids blind Google scheduling. Ninety-nine selected products have 180 effective destinations; one has none. Rakuten is excluded from paid demand because current rights support only source-local read-only projection.

Maximum derived demand is up to 65 Amazon Products tasks (`$0.0975`), 22 Amazon Sellers tasks (`$0.0330`), and two Google Sellers tasks (`$0.0020`), separately governed. The theoretical `$0.1325` total needs at least three `$0.0500` UTC-day windows and is not an authorization. The certified selector and stage-specific size policy subsequently selected the exact canary recorded above; they do not authorize or select any larger slice.
...
The reserved final six remained exactly the committed deterministic partition. Cohort `config/mercury/scale-ramp/25-product-slice-2-amazon-identity.json`, plan `mer_iddiscplan_e0ec35a7e9f99bd15f824e60`, authorization `mer_iddiscauth_7521266859ea7c8df1ee71da`, and run `mer_iddiscrun_9d4088c5bc66e361978c27dc` are terminal `COMPLETED`. Six unique Amazon Products tasks spent exactly `$0.0090`; five members established `STRONG_UNIQUE_ASIN` and `ram_kingston_kf432c16bbk2_16` established `INSUFFICIENT_ASIN_EVIDENCE` with H052 review available. Two zero-paid-work RESUME operations reused the six tasks and completed retrieval/finalization. No retry, duplicate, provider/systemic failure, Sellers task, H052 action, evidence/history, or downstream authority occurred. September 15 spend is `$0.0325` under the `$0.0500` ceiling.

The complete 19-attempt breadth stage produced 14 strong identities (`73.7%`), three multiple-compatible outcomes (`15.8%`), one variant conflict (`5.3%`), and one insufficient-evidence outcome (`5.3%`), using 19 tasks and `$0.0285`. There were zero provider/systemic failures, retries, duplicate tasks, or engineering interventions. These measured stage results are not a catalog-wide forecast. Longitudinal work remains separate and must not begin until a distinct stage-gate review.
...
The authoritative execution ledger owns actual spend. Authorization and execution both revalidate durable UTC-day spend. The operational ceiling is `$0.0750`; Google `PRODUCTS`/`SELLERS` remains capped at `$0.001` per task and Amazon `AMAZON_PRODUCTS`/`AMAZON_SELLERS` at `$0.0015` per task. Automatic paid retries remain zero. The ceiling is not an architectural maximum and creates no authority to spend.
...
At `$0.0750` per UTC day, the theoretical maximum is 75 Google tasks or 50 Amazon tasks when a clean day is devoted entirely to one task class. Actual bounded slices must also fit remaining ledger capacity and their own cohort limits. The prepared 26-task experiment has a `$0.0380` analytical maximum, leaving `$0.0370` on a clean day. Unused capacity is not authority; any future ceiling change requires another governance decision supported by operating evidence.
...
| Spend governance | GREEN | Ledger-owned actual spend, same-parent progression, dual revalidation | Govern at `$0.0750`; slice work |
...
|---|---|---|
| P0 before 25 | None | — |
| P1 before next larger 100-stage slice | CLOSED: direct selector-proposal → identity-discovery PREPARE binding is fixture- and production-shaped-certified; existing selector and PREPARE owners remain authoritative | Existing selector + identity-discovery PREPARE composition |
...
Fresh reassessment confirmed 24 repeat-ready pairs across 22 products. Post-25 Longitudinal Canary 1 (`config/mercury/longitudinal/post-25-longitudinal-canary-1.json`) is the first longitudinal scale-ramp cohort after the completed 25-product identity-breadth gate; it does not supersede MERCURY-HISTORY-056 or the September 14 first bounded repeat run. Plan `mer_repeatrunplan_3ca259b43b481fddd89e4759`, authorization `mer_repeatrunauth_4b0db03039226f7f3aee0881`, and run `mer_repeatrun_e3d2417b9e15058c2b44b649` governed six unique tasks with `$0.0080` actual spend and zero retries. One finite P1-A invocation reused only those tasks and added no paid work.

The run remains terminal `COMPLETED_WITH_EXCEPTIONS`: three members originally completed with 12 evidence/history facts and three Amazon members stopped on `DATAFORSEO_AMAZON_MONEY_INVALID`. D1 fixture-certifies structured delivery money without permissive coercion. The generic immutable-result reprocessing boundary subsequently applied the current certified source processor to those exact three results with no retrieval, task, retry, or spend. It appended three governed actions and effective effects of Crucial 7 evidence / 6 history (one retained row remained history-ineligible for missing item price), G.Skill 14/14, and CMK32 3/3; totals are 75 evidence and 69 history. The original run/member outcome is unchanged, exact replay is idempotent, and canonical/review/E2S/publication/current-price authority remains unchanged. D2 now projects original lifecycle counts and current effective reprocessing as separate read-only dimensions.
