...
| Repeat observation | GREEN | Canonical owner projects 24 pairs (22 Amazon, 2 Google); six-pair plan `mer_repeatrunplan_3ca259b43b481fddd89e4759` is prepared/inspected, not authorized or started | Review the exact `$0.0080` authorization envelope separately |
...
Fresh reassessment confirmed 24 repeat-ready pairs across 22 products. Post-25 Longitudinal Canary 1 (`config/mercury/longitudinal/post-25-longitudinal-canary-1.json`) is the first longitudinal scale-ramp cohort after the completed 25-product identity-breadth gate; it does not supersede MERCURY-HISTORY-056 or the September 14 first bounded repeat run. It selects six pairs: two same-product Amazon/Google controls plus DDR4/DDR5, DIMM/SODIMM, kit/single-module, three-manufacturer, and existing/zero-history coverage. Plan `mer_repeatrunplan_3ca259b43b481fddd89e4759` is six READY, zero blocked, four Amazon plus two Google tasks, `$0.0080` maximum spend, zero retries, `NOT_AUTHORIZED`, and `NOT_STARTED`; current-day spend at inspection was `$0.0000` under `$0.0500`.

Before any authorization, review that exact envelope separately. If later authorized, measure task creation and completion, pending and retrieval duration, P1-A invocations/attempts/bound outcomes and zero-paid-work proof, authorized versus actual cost and daily progression, evidence/history amplification and replay, relevant store growth/read-write latency, operator actions and exception classes, and Forge sufficiency. P1-B currently projects started repeat runs only, so prepared-only plan visibility remains a measured future capability. Do not authorize or start merely to make the plan visible.
