...
The reserved final six remain exactly the committed deterministic partition. Current reassessment at `2026-09-15T22:28:02.044Z` found all 6 `READY_FOR_DISCOVERY` with unchanged Amazon rights and active reviewed destinations. Cohort `config/mercury/scale-ramp/25-product-slice-2-amazon-identity.json` prepared plan `mer_iddiscplan_e0ec35a7e9f99bd15f824e60`: six tasks / `$0.0090`, `$0.0235` current-day spend, `$0.0325` projected final spend under the `$0.0500` ceiling, `NOT_AUTHORIZED`, and `NOT_STARTED`. PREPARE/INSPECT performed no provider operation or paid work. Authorization requires a separate operator increment. The separate longitudinal control should wait until `AFTER_25_IDENTITY_STAGE` so it does not compete for the final breadth slice's budget.
