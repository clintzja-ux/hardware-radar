...
Fresh reassessment confirmed 24 repeat-ready pairs across 22 products. Post-25 Longitudinal Canary 1 (`config/mercury/longitudinal/post-25-longitudinal-canary-1.json`) is the first longitudinal scale-ramp cohort after the completed 25-product identity-breadth gate; it does not supersede MERCURY-HISTORY-056 or the September 14 first bounded repeat run. It selects six pairs: two same-product Amazon/Google controls plus DDR4/DDR5, DIMM/SODIMM, kit/single-module, three-manufacturer, and existing/zero-history coverage. Plan `mer_repeatrunplan_3ca259b43b481fddd89e4759` is six READY, zero blocked, four Amazon plus two Google tasks, `$0.0080` maximum spend, and zero retries. All six current identity and source-rights bindings revalidated. Single-use authorization `mer_repeatrunauth_4b0db03039226f7f3aee0881` was created at `2026-09-16T03:08:03.413Z`, expires at `2026-09-16T04:00:00.000Z`, and leaves the run `NOT_STARTED`; current-day spend remained `$0.0000` under `$0.0500`.

Authorization created no provider call, paid task, execution, result, evidence, history, canonical observation, or downstream authority. Before any START, separately review this exact authorization and its expiry. If explicitly approved and still valid, measure task creation and completion, pending and retrieval duration, P1-A invocations/attempts/bound outcomes and zero-paid-work proof, authorized versus actual cost and daily progression, evidence/history amplification and replay, relevant store growth/read-write latency, operator actions and exception classes, and Forge sufficiency. P1-B currently projects started repeat runs only, so authorized-but-not-started visibility remains a measured future capability. Do not start merely to make the plan visible.
