...
`BoundedRepeatObservationRunService` is the repeat-domain facade over the shared Mercury `NeutralBoundedPaidActionCoordinator` and the explicit `RepeatObservationBoundedDomainAdapter`. PREPARE RUN accepts only an explicit UTC cycle and product/source pairs, persists READY and blocked results as one immutable plan, and calculates the exact task/spend envelope. One expiring neutral parent authorization binds the complete member set and spend envelope. It contains no historical-admission authority. START consumes that scope once and the repeat adapter derives exact, non-expandable child task authority; it cannot add products, sources, discovery, retries, or tasks.

The coordinator knows members only by opaque `memberKey` and owns plan identity, cohort digest, aggregate/per-member ceilings, expiry, single-use START, shared-spend revalidation, member coordination, waiting/resume, failure isolation, and aggregate terminal state. The repeat adapter alone validates `SELLERS` / `AMAZON_SELLERS`, reusable identity, rights, retention, and H058 factual admission. Its child authorization retains `historicalFactAdmissionAuthorized=true`; the neutral parent does not.

Every new repeat child is durably bound in its run-member payload to the exact parent authorization ID/digest, plan/member and repeat preparation, source, operation, task ceiling, child authorization ID/digest, zero retries, and an expiry no later than the parent. Changed derivation conflicts fail closed. This content-addressed binding—not operator text or naming convention—proves delegated task authority.

Run plans, authorizations, runs, and minimal member progress use indexed tables in the same `repeat-observations.sqlite` database. A compatibility persistence adapter projects neutral `planId`, `authorizationId`, `cycle`, and `memberKey` onto the established repeat columns; no database, table, or historical record is migrated or rewritten. `WAITING_FOR_PROVIDER` is durable and RESUME retrieves only already-created tasks. Product-local evidence/offer failures become isolated exceptions; rights, binding, budget, repository, authorization, SQLite, or task-owner integrity failures stop the run. The small states are `PREPARED`, `AUTHORIZED`, `RUNNING`, `WAITING_FOR_PROVIDER`, `COMPLETED`, `COMPLETED_WITH_EXCEPTIONS`, and `FAILED`.
