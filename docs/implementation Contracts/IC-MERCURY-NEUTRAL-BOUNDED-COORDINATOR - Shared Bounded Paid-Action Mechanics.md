...
Fixture-certified with repeat-observation parity, neutral SQLite persistence, and source-owner child-authority prerequisites. Products identity-discovery composition is deliberately not implemented.
...
Each adapter-derived child has a durable content-addressed binding over the parent authorization ID/digest, plan ID/digest, opaque member key, immutable domain member ID, source, operation, task ceiling, zero retries, child expiry, and child-binding digest. Child expiry cannot exceed parent expiry. Exact derivation is deterministic; changed binding fails closed. Google Products and Amazon Products authorization creation now accept this typed derivation envelope as an optional second authority origin, validate it against the exact parent/plan/member and their own source ceiling, and preserve their existing manual/operator origin unchanged. The neutral parent never replaces source rights, acceptance artifacts, task authorization, task consumption, or spend revalidation.

## Persistence and compatibility

The coordinator depends on a small operation-neutral repository interface with direct plan, authorization, run, member, and parent-child operations. `SqliteNeutralBoundedRepository` adds operation-neutral, indexed tables to the same operational SQLite database for plans, plan members, parent authorizations, runs, run members, and child references. It contains no repeat-observation columns or Products policy. Existing repeat tables and their compatibility adapter continue unchanged, preserving transactions, replay, concurrency, and the first production run without migration or rewrite. Other domains may be composed later through an explicit adapter; this increment creates no Products adapter, task ledger, spend ledger, identity repository, evidence repository, or history repository.
...
Fixtures cover direct indexed lookup and replay/conflict behavior through 10, 100, 1,000, and 10,000 neutral members. No provider operation, paid task, production authorization/run, evidence/history mutation, or downstream mutation occurred during extraction or prerequisite certification.
