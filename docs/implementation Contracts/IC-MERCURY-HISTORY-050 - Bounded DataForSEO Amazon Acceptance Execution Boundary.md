...
## H050F Amazon retrieval runtime composition

The first corrected recovery execution created Amazon Products task `09130222-2304-0209-0000-b10932d7d33d` for `$0.0015`. Its subsequent retrieval failed locally because the shared production retrieval owner always instantiated the Google `DataForSeoMerchantApiClient` before wrapping it in the Amazon acquisition service. That client has no Amazon result methods. H050F makes the existing retrieval owner select `DataForSeoAmazonMerchantApiClient` for every `AMAZON_*` operation, matching H050B's POST composition, and passes the same injected credentials/transport through the production acceptance CLI runtime.

Products, ASIN, and Sellers resolve respectively to the certified Amazon advanced task-get endpoints. Google retrieval continues to use the Google client. Retrieval derives the existing durable task, creates no task or paid action, costs `$0`, and persists only an available result through the existing immutable provider-result repository. Pending results remain pending, terminal provider failure remains distinct, exact material replay is idempotent, and no identity processing occurs automatically.

The real task is `TASK_CREATED_RETRIEVAL_PENDING_AFTER_LOCAL_CLIENT_FAILURE`: it remains immutable and is the only correct retrieval target. No result was persisted by the failed local attempt, and certification performs no live GET.
