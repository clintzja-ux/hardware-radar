...
The operational default is `$0.0750` per UTC day. `createAcquisitionBudgetPolicy` remains the canonical configuration owner and accepts an explicit ceiling for fixture or future operator-controlled composition; production DataForSEO authorization paths consume its exported default rather than scattering a replacement literal. This scale-ramp limit is internal Hardware Radar governance, not a provider API limit or permanent architectural maximum. It is supported by supervised evidence of bounded accounting, single-use authorization, durable ledger ownership, zero paid retries, member-local isolation, and deterministic recovery. It changes neither the `$0.001` Google Shopping task ceiling nor the `$0.0015` Amazon task ceiling, and automatic paid retries remain zero. It creates no acquisition authority; the increase from `$0.0500` was a separate operator-approved governance decision for the prepared larger longitudinal experiment.
