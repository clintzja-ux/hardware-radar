...
Production H052 resolves the Products task across the existing canonical Amazon result owners because an acceptance artifact may have been executed either by the standalone acceptance runtime or by the neutral Products-discovery runtime. The composition is a read-through view, not another repository: new standalone results still write only to the existing acceptance-result owner, while an existing task must resolve to exactly one canonical result across the acceptance and Products-discovery stores. Zero or multiple matches fail closed. H052 still requires exact canonical result ID, result digest, task, artifact, product, rights, and outcome lineage; repository routing never weakens immutable-result validation.
