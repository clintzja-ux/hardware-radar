...
The source-specific `NEWEGG_MANUAL_PUBLISHER_OBSERVATION` and `AMAZON_MANUAL_PUBLISHER_OBSERVATION` lanes permit bounded human publisher observations to establish fresh factual item-price display and comparison when exact product/destination, positive USD price, explicit availability, observation time, rights, and private operator provenance are valid. Condition, seller, shipping, tax, and fees remain unknown and are never inferred; known incompatibilities fail closed. Separately governed fact-level history preserves the manual provenance and null ancillary facts through the existing historical repository. Neither lane grants publication/release authority. The first dual-retailer snapshot executions are complete; the next gate is read-only comparison proof followed by separately reviewed history admission.
