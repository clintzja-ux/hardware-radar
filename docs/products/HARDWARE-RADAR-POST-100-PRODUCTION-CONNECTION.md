...
**Active gate:** `MANUAL_CURRENT_DISPLAY_SNAPSHOT_EXECUTION_COMPOSITION_P1` fixture-certified; real execution remains unauthorized
...
Manual and automated acquisition are peer governed lanes, not successive snapshot architectures. The same product may carry any independently qualified Amazon/Newegg combination: manual/manual, manual/automated, automated/manual, or automated/automated. Each offer retains its actual source identity. No generic rule makes manual, automated, affiliate, lowest-price, or newest evidence win. Unresolved same-retailer source collisions preserve both provenance chains at their evidence owners and fail closed into governed conflict review. Loss, staleness, or absence of one retailer/source does not suppress another retailer's qualified current observation.
...
Amazon destinations and reusable identities exist, and Mercury has immutable evidence/history. Those facts do not establish a current publicable offer. Existing DataForSEO Amazon history does not confer current authority on another source lane. Separately, `AMAZON_MANUAL_PUBLISHER_OBSERVATION` permits bounded human public-page research for current item-price display/comparison and separately governed fact-level history. It grants no Amazon API, DataForSEO, scraping, delivered-cost, Cheapest, or publication authority.
...
The source-specific `NEWEGG_MANUAL_PUBLISHER_OBSERVATION` and `AMAZON_MANUAL_PUBLISHER_OBSERVATION` lanes permit bounded human publisher observations to establish fresh factual item-price display and comparison when exact product/destination, positive USD price, explicit availability, observation time, rights, and private operator provenance are valid. Explicitly observed condition, seller, shipping, and research-URL provenance are preserved; genuinely absent tax, fees, or ancillary facts remain unknown and are never inferred, and known incompatibilities fail closed. Separately governed fact-level history preserves that manual provenance and exact known/unknown facts through the existing historical repository. Neither lane grants publication/release authority. Complete reviewed observations may progress routinely after deterministic revalidation; exceptional records retain the explicit review path.
