...
The source-specific `NEWEGG_MANUAL_PUBLISHER_OBSERVATION` and `AMAZON_MANUAL_PUBLISHER_OBSERVATION` lanes permit bounded human publisher observations to establish only fresh factual item-price display when exact product/destination, positive USD price, explicit availability, observation time, rights, and private operator provenance are valid. Condition, seller, shipping, and fees remain unknown. Such offers are not comparison-eligible and cannot become lower-price comparison or Cheapest. The lanes create no history and grant no publication/release authority. PREPARE/INSPECT and the separate exact-preparation snapshot authorization/execution composition are fixture-certified; two real preparations exist, but neither is authorized or executed.
