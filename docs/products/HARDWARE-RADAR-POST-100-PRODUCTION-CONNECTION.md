...
Newegg is canonical retailer `RETAILER-0004`, has broad reviewed destination coverage, and the checked-in current-retail artifact contains four formerly eligible offers. At the current date those observations are stale and cannot be reused as current prices. `IC-RAKUTEN-NEWEGG-CURRENT-COMMERCE-RIGHTS-001` now establishes acquisition, ephemeral/current-state retention, public current-price display, and cross-retailer item-price comparison for the approved US Product Catalog path. Feed-derived history, analytics, recommendation, redistribution, seller/condition inference, and publication authority remain fail closed. Rakuten parsing and deterministic source-local FULL/DELTA projection are certified; production refresh composition and offer classification remain separate gates.
