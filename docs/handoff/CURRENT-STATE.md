...
Current growth increment:        GROWTH-005A — Mercury ordinary retailer destination contract implemented and fixture-certified; zero production destinations and no public rendering
...
GROWTH-002 adds a deterministic public Atlas projection and static `/ram/` discovery surface for all 22 canonical RAM products. GROWTH-003 establishes each Atlas product ID plus its validated Atlas slug as one persistent public identity at `/ram/<slug>/`, generates 22 factual specification pages, links every catalog card to its page, and adds the same route set to the deterministic sitemap. GROWTH-004 adds one `/ram/compare/` shell whose ordered, shareable query resolves exactly two known Atlas public slugs and presents factual `SAME`, `DIFFERENT`, and `NOT_AVAILABLE` specification states. Catalog and product-detail actions may prefill one slot; no hidden browser state is required. The comparison shell is noindex and canonicalizes to its single base route, which is the only comparison route in the sitemap. GROWTH-005A adds the immutable Mercury `RetailerDestination` contract and file repository for exact operator-reviewed ordinary product-page navigation. Destination validity requires active/ready Atlas product identity, active Atlas retailer identity, exact MPN and marketplace/domain binding, canonical HTTPS URL, evidence references, and append-only lifecycle provenance. It grants only navigation eligibility, is independent of affiliate state and data rights, and contains no offer or market observation fields. There are zero production destination records and no public destination rendering. Search/filter behavior, the five-item primary navigation, and the price-first homepage remain unchanged. These Atlas projections and the destination boundary create no market claim, compatibility verdict, Cheapest, Pick, ranking, recommendation, or publication authority. Shipped slugs are persistent; any genuine future correction requires an explicit alias/redirect migration rather than silent URL churn.
...
| **Public Hardware Radar** | Generated static RAM site, Atlas-driven `/ram/` catalog, 22 canonical product-reference routes, and one factual `/ram/compare/` shell, plus category/trust pages under `public/`; consumes a governed winner plus bounded alternatives, preserves listed-price and shipping-knownness semantics, and remains fail closed without qualifying published observations | Launch shell and static catalog/detail/comparison projections are truthful and fixture-tested but still have zero eligible public observations; specification comparison creates no market or recommendation authority; production publication remains later controlled work; current external hosting/deployment was not queried by this task |

## Current test baseline

The current runners declare **220 subsystem test files**:
