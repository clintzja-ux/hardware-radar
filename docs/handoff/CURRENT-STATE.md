...
MERCURY-ACTIVATION-002 fixture-certifies a zero-spend `ACQUISITION_ORCHESTRATION_AUTHORITY` over the existing task-specific DataForSEO boundaries. MERCURY-ACTIVATION-002B fixture-certifies its thin production PREPARE composition: canonical Atlas loading, historically governed provider-identity reuse, source-rights binding, existing-ledger UTC-day spend binding, deterministic operator-local persistence, and independent inspection. Portfolio PREPARE derives the current Atlas inventory rather than duplicating a product list: 26 canonical RAM products, 11 `ACTIVE` + `READY` acquisition-eligible products, and 15 lifecycle-excluded products. Two eligible products reuse governed provider identity and begin at `SELLERS`; nine begin at `PRODUCTS`, producing a computed maximum envelope of 29 tasks / `$0.029` under the unchanged `$0.001` task and `$0.010` UTC-day ceilings with zero automatic paid retries. The immutable portfolio and separate event projection can recommend the next task PREPARE but grant no paid execution, result acceptance, downstream Mercury, destination, or public authority. One authorized zero-spend production artifact now exists only in ignored operator state as cycle `mer_acqportfolio_14bad432b919feca4cc416a0`, prepared at `2026-09-03T17:59:18.006Z`; it recorded `$0.000` current UTC-day spend and `$0.010` remaining capacity. Paid acquisition has not been executed for this portfolio, no provider task or paid authorization was created, actual spend remains `$0.000`, and the first paid cycle still requires operator review plus the existing separate task-level authorization.
