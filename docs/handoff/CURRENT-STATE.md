...
Current growth increment:        GROWTH-003 — canonical public product identity and 22 Atlas RAM detail routes implemented and fixture-certified
...
GROWTH-002 adds a deterministic public Atlas projection and static `/ram/` discovery surface for all 22 canonical RAM products. GROWTH-003 establishes each Atlas product ID plus its validated Atlas slug as one persistent public identity at `/ram/<slug>/`, generates 22 factual specification pages, links every catalog card to its page, and adds the same route set to the deterministic sitemap. Search/filter behavior, the five-item primary navigation, and the price-first homepage remain unchanged. The projection and pages contain no Mercury observations, retailer/affiliate fields, market claims, Cheapest, Pick, ranking, or publication authority. Shipped slugs are persistent; any genuine future correction requires an explicit alias/redirect migration rather than silent URL churn.
...
| **Public Hardware Radar** | Generated static RAM site, Atlas-driven `/ram/` catalog and 22 canonical product-reference routes, plus category/trust pages under `public/`; consumes a governed winner plus bounded alternatives, preserves listed-price and shipping-knownness semantics, and remains fail closed without qualifying published observations | Launch shell, static catalog/detail projection, and comparison projection are truthful and fixture-tested but still have zero eligible public observations; production publication remains later controlled work; current external hosting/deployment was not queried by this task |
