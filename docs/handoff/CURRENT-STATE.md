...
Current growth increment:        GROWTH-002 — static Atlas RAM catalog search/filter surface implemented and fixture-certified at `/ram/`
...
GROWTH-002 adds a deterministic public Atlas projection and static `/ram/` discovery surface for all 22 canonical RAM products. Search covers brand, family/name, model, and exact MPN; filters derive from governed Atlas attributes and combine without recommendation or compatibility claims. The projection contains no Mercury observations, retailer/affiliate fields, prices, availability, shipping, condition, Cheapest, Pick, or ranking authority. The five-item primary navigation and price-first homepage remain unchanged; a restrained footer link owns catalog discovery. Full product-detail routes remain deferred.
...
| **Public Hardware Radar** | Generated static RAM site, Atlas-driven `/ram/` catalog, and category/trust pages under `public/`; consumes a governed winner plus bounded alternatives, preserves listed-price and shipping-knownness semantics, and remains fail closed without qualifying published observations | Launch shell, static 22-product catalog, and comparison projection are truthful and fixture-tested but still have zero eligible public observations; production publication remains later controlled work; current external hosting/deployment was not queried by this task |
