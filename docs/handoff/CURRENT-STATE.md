...
B-001 maps all 22 canonical RAM products to the existing exact-MPN Mercury path. The original Corsair anchor is the sole `ACTIVE/READY` operational candidate; the other 21 are blocked only at the current Atlas `DRAFT/PENDING` lifecycle gate, not by a demonstrated Mercury code defect. A seven-product representative cohort spans all public RAM categories, all five launch brands, DDR4/DDR5, DIMM/SODIMM, kits/single modules, and clean/punctuation-heavy MPNs. B-001 authorizes no acquisition or spend. The next boundary is a separate lifecycle decision for cohort products, followed by zero-spend candidate inspection and—only if separately authorized—one governed PRODUCTS task for `ram_crucial_cp2k16g56c46u5` capped at one task / `$0.001`.
