...
Current acquisition operation: B-010A — projection-aware production promotion composition fixture-certified; production reassessment not run
...
Current retailer-registration increment: B-011A — `RETAILER-0003` MemoryC canonically registered; merchant approval not performed
...
B-011/C-002A fixture-certified MemoryC canonical retailer readiness, and B-011A registered exactly `RETAILER-0003` in Atlas. MemoryC-controlled sources establish the active `MemoryC` / `MemoryC.com` storefront at `memoryc.com`, operated by GoBeyond Ltd. with a MemoryC Inc. US presence. Exact `memoryc.com` and `www.memoryc.com` evidence resolves while lookalikes and unrelated subdomains do not. Atlas now contains three retailers; Amazon and Platinummicro remain unchanged. In isolated Crucial evidence, the existing governed product projection remains `VERIFIED`, hypothetical separately reviewed merchant registration becomes `REGISTERED`, and E2G/E2H derives `HISTORICAL_ELIGIBLE`; E2J remains separate and was not run. No merchant decision was created, retained evidence remains unchanged, condition and shipping remain unknown, and all MemoryC-specific rights, affiliate, feed/API, adapter, freshness, E2S, comparison, recommendation, canonical, publication, Current Price, Cheapest, and Pick authority remain absent. The next safe boundary is a read-only MemoryC merchant-review PREPARE inspection followed by separately authorized merchant governance.
