...
MERCURY-ACTIVATION-002 fixture-certifies a zero-spend `ACQUISITION_ORCHESTRATION_AUTHORITY` over the existing task-specific DataForSEO boundaries. Portfolio PREPARE derives the current Atlas inventory rather than duplicating a product list: 26 canonical RAM products, 11 `ACTIVE` + `READY` acquisition-eligible products, and 15 lifecycle-excluded products. Two eligible products reuse governed provider identity and begin at `SELLERS`; nine begin at `PRODUCTS`, producing a computed maximum envelope of 29 tasks / `$0.029` under the unchanged `$0.001` task and `$0.010` UTC-day ceilings with zero automatic paid retries. The immutable portfolio and separate append-only progress projection can recommend the next task PREPARE but grant no paid execution, result acceptance, downstream Mercury, destination, or public authority. No production portfolio artifact or provider task was created, actual spend remains `$0.000`, and the first paid cycle still requires explicit operator review and separate task-level authorization.
