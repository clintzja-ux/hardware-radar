...
- Apply **Retail Commerce Source Independence**: Hardware Radar must not depend on any single retailer feed, affiliate network, API, provider, or acquisition mechanism for continued operation. Automated feeds improve scale and freshness; governed manual observation is a legitimate source-specific fallback/bootstrap path where supported. Losing one source degrades only its dependent capabilities and must not invalidate Atlas, unrelated Mercury evidence, other retailer paths, or the public catalog. Affiliate state must never influence Cheapest or Picks. See ADR-060 and `IC-RETAIL-SOURCE-RESILIENCE-001`.
...
- Keep manual publisher observations and automated commerce feeds as distinct Mercury source lanes. Exact-product current item-price comparison may remain truthful while condition, seller, shipping, tax, or fees are unknown; unknown values stay unknown, known incompatibilities fail closed, and private operator provenance never enters the public artifact. Manual historical facts use the same durable Mercury history contract with distinct provenance and separate admission authority.
- Treat complete `Ready = YES` reviewed manual observations as submitted for deterministic routine Mercury progression: revalidate every current binding and claim, preserve current/history independence, and require governed human review only for exceptions. Routine progression may reuse existing authorization/execution records as audit lineage without a second per-record human ceremony; it grants no publication, artifact, release, or recommendation authority.
- Current Display is not publication authority, publication authority is not an artifact, an artifact is not release, and release is not deployment. Mercury's product-scoped candidate/authorization and immutable artifact materialize/inspect boundaries revalidate exact facts, preserve lineage, and reuse the existing public renderer; Sentinel release control accepts current-display authority only through a persisted verified artifact. Item-price ordering is legitimate when comparison-eligible and never implies delivered-cost superiority.
- Treat rights provenance and action authority as separate time-indexed facts. Immutable evidence retains the source-rights digest bound when it was prepared; later actions independently require current claim-specific permission and record the current authorizing digest. Policy evolution never rewrites old evidence and never makes unverifiable evidence valid.
...
- The first post-100 production-connection gate is now an implemented and fixture-certified fail-closed static publication release control. Its repository/build-governed manifest defaults `OFF`, binds one exact sanitized current-retail artifact and Preview or Production environment, and remains separate from qualification, publication, and deployment authority. This delivery choice preserves the canonical hybrid acquisition doctrine in the [product evolution document](../products/HARDWARE-RADAR-PRODUCT-EVOLUTION.md): independent market intelligence and retailer/affiliate/commerce current-display inputs are complementary, separately governed Mercury lanes and neither inherits the other's authority. Gateway is not required for the initial static canary and must not become an on-demand price-acquisition proxy; Beacon may follow independently because existing GA4 measurement is sufficient for the first bounded experiment. See the [post-100 production connection plan](../products/HARDWARE-RADAR-POST-100-PRODUCTION-CONNECTION.md).
...
- Public rendering fails closed on an unsupported claim, not on the entire product experience. Atlas-backed catalog and specification surfaces remain useful when current prices are absent, stale, or unavailable.
- The explicit local development-preview build may render canonical governed development state for UI verification without creating publication, artifact, release, or deployment authority. Production output remains independently release-manifest gated; development preview is never a production fallback.
- Evidence and audit records are immutable or append-only. Replay is deterministic; conflicting replay fails closed.
- Condition is an offer-level claim: `UNKNOWN` must never be inferred as `NEW`. For the ephemeral display stream only, policy `RETAIL-DISPLAY-002-1.0` may derive `NEW` from an exact standard Amazon or Newegg retail product page when no used, refurbished, renewed, open-box, pre-owned, replacement-only, marketplace-ambiguity, mixed-condition, or contradictory signal exists; search results and retailer reputation alone are insufficient. Any supplemental durable condition evidence must remain additive and preserve exact offer binding, provenance, independent source rights, temporal compatibility, and contradiction handling without rewriting retained, historical, or canonical observations.
...
- The approved US Rakuten/Newegg Product Catalog path may acquire and maintain ephemeral current commerce state and may support public current-price display and cross-retailer item-price comparison after independent identity, seller/condition, destination, freshness, comparability, publication, and release gates pass. These current-commerce rights grant no feed-derived history, analytics, recommendation, redistribution, or automatic `NEW` inference; see `IC-RAKUTEN-NEWEGG-CURRENT-COMMERCE-RIGHTS-001`.
- The public current-retail projection may call the lowest fresh qualifying displayed item price **Cheapest RAM Today** only under `PUBLIC-RAM-CURRENT-RETAIL-001-1.0` (36 hours) and with the exact shipping/tax/fee exclusion disclosure. It is not delivered-cost Cheapest, does not read or write Mercury history, and grants no publication, recommendation, or Pick authority.
...
- Prepared Mercury acquisition portfolios retain their immutable PREPARE-time membership and spend envelope when Atlas lifecycle eligibility changes; newly eligible products require a future portfolio PREPARE.
