...
Mercury derives observation freshness from immutable observation timestamps, an explicit versioned freshness policy, optional absolute expiration, and an explicit evaluation time.

Freshness status is not persisted as canonical observation truth.
...
- Core freshness calculation does not call `Date.now()` implicitly.
- Evaluation requires an explicit `evaluatedAt` value or equivalent injected clock at orchestration level.
...
- Retailer-specific production thresholds require explicit policy decisions.
